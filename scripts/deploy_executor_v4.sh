...
#
# ПРАВКА (пятый раунд ревью, пункт 4): предыдущий пример в этом
# комментарии (`VAR=... curl ... | sudo -E bash`) БЫЛ ОШИБОЧЕН --
# `VAR=значение команда1 | команда2` присваивает VAR ТОЛЬКО для
# команда1 (здесь -- curl), НЕ экспортирует его в окружение вообще, и
# `sudo -E` тут не помогает: ему просто НЕЧЕГО "сохранять" -- VAR
# никогда не попадал в окружение вызывающей оболочки, значит и в
# окружение процесса ПОСЛЕ sudo (то есть в тот самый bash, что читает
# скрипт из stdin) он попасть не может.
#
# ПРАВИЛЬНО -- явно задать переменные ДЛЯ ПРОЦЕССА, который выполняет
# скрипт (`env VAR=знач ... КОМАНДА` -- сам env, а не sudo/pipe,
# отвечает за то, что именно унаследует КОМАНДА и её дочерние процессы,
# включая curl и pipe-получателя):
#
#   sudo -u bot env BRANCH=<commit-sha> bash -c \
#     'curl -fsSL https://raw.githubusercontent.com/russiansky777/robinhood-chain-alpha/<commit-sha>/scripts/deploy_executor_v4.sh | bash'
#
# Здесь `env BRANCH=<sha> bash -c '...'` запускает bash -c С УЖЕ
# УСТАНОВЛЕННЫМ BRANCH в его собственном окружении -- curl и
# пайп-получатель (второй bash, читающий stdin) -- ОБА дочерние
# процессы ЭТОГО bash -c и наследуют его окружение целиком, включая
# BRANCH. commit-sha подставляется ОДИН РАЗ в обоих местах (URL И
# BRANCH=), поэтому байткод (скачанный ВНУТРИ скрипта по этому же
# BRANCH) гарантированно с ТОЙ ЖЕ версии, что и сам исполняемый скрипт.
#
