# Ночь 25.09: боевое и исследование (доклад собран 2026-09-25T03:38:03Z)

## 1. Боевое

**Решения по сигналам.** Покупок 8; по кодам: {"NOT_A_BUY": 168, "TARGET_AMOUNT_OUT_OF_RANGE": 104, "SKIP_TARGET_INCREASE_POSITION": 103, "TOKEN_RECEIVED_NOT_BOUGHT": 80, "BUY": 8, "SKIPPED_DUP_MINT": 2, "THRESHOLD_EDGE": 1, "SKIP_TAXED_ROUTE": 1}. Полоса идёт только там, где мы покупаем, поэтому её тишина объясняется этой строкой.

**Полоса своей отправки.** Путей 2, отправлено 0, отказ по симуляции (SKIP_SIM_FAIL) 0. Позиций полосы 0, закрыто 0, UNSOLD 0. Итог по закрытым: — (по 0 сделкам). Медиана от отправки до появления в потоке: —.
Где путь останавливался: build -- 2. Причины: котировка пула не SOL -- полоса только одношаговая (2).

**Пары «Bloom против нашей».** Пар 0, из них мы раньше 0; медиана разницы — (плюс -- мы раньше).

**Тень.** Записей 79, по вердиктам: {"would_pass": 19, "нет вердикта": 60}. Медиана сборки 0.82 мс, симуляции 15.9 мс.
...
{
  "ok": true,
  "built_utc": "2026-09-25T03:38:03Z",
  "pickaxe": {
    "ok": true,
