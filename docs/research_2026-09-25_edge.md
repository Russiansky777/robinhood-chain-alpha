# Ночь 25.09: боевое и исследование (доклад собран 2026-09-25T04:11:21Z)

## 1. Боевое

**Решения по сигналам.** Покупок 9; по кодам: {"NOT_A_BUY": 172, "TARGET_AMOUNT_OUT_OF_RANGE": 105, "SKIP_TARGET_INCREASE_POSITION": 104, "TOKEN_RECEIVED_NOT_BOUGHT": 88, "BUY": 9, "SKIPPED_DUP_MINT": 2, "THRESHOLD_EDGE": 1, "SKIP_TAXED_ROUTE": 1}.

**Полоса своей отправки.** Путей 3, отправлено 0, отказ по симуляции (SKIP_SIM_FAIL) 0. Позиций полосы 0, закрыто 0, UNSOLD 0. Итог по закрытым: — (по 0 сделкам). Медиана от отправки до появления в потоке: —.
Где путь останавливался: build -- 3. Причины: котировка пула не SOL -- полоса только одношаговая (3).

**Пары «Bloom против нашей».** Пар 0, из них мы раньше 0; медиана разницы — (плюс -- мы раньше).

**Тень.** Записей 82, по вердиктам: {"would_pass": 20, "нет вердикта": 62}. Медиана сборки 0.84 мс, симуляции 15.9 мс.
...
{
  "ok": true,
  "built_utc": "2026-09-25T04:11:21Z",
  "pickaxe": {
    "ok": true,
  ...
  "summary": {
    ...
    "total_bloom_trades": 32,
    "collected": 2,
    "collected_share": 0.0625,
    "with_diff_pct": 0,
