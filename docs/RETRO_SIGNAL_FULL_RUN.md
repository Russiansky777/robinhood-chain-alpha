...
Починено: `compare_prices()` считает сверенные пары, вердикт `совпало` становится `null` при нуле сверенных пар, `self_check.ok` становится `null` с явной запиской. Добавлен режим `--recheck` — пересверка по готовому файлу со своим коротким бюджетом, чтобы проверка не делила бюджет с расчётом.
