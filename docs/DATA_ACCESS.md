...
## Инцидент: 402 на шаге 1 (2026-08-31)

Реальный прогон через GitHub Actions (после того как схема была
исправлена по данным `analysis/_probe_schema.py`) дошёл до первого
содержательного запроса пайплайна — `sql/01_pool_creation_blocks.sql`
("pool creation blocks") — и получил от Dune **402 Payment Required**.
По правилу "не тратить кредиты молча" пайплайн остановился сам (см.
`dune_client.DuneCreditsExhausted`), без ретраев.

**Что успело выполниться до этого:** только диагностические probe-запросы
(`analysis/_probe_schema.py`, 5 маленьких запросов к
`information_schema.tables` — десятки-сотни строк метаданных, не свопы) —
они прошли успешно и почти наверняка стоили копейки. Ни одной строки
реальных данных по свопам/PnL за июль или август выгружено не было —
пайплайн упал на самом первом шаге, до какой-либо агрегации по кошелькам.

**Точный остаток кредитов на счету не виден** — Dune API v1 не отдаёт
баланс кредитов через `/execution/.../results` (только execution stats
вроде количества строк/datapoints), а сайт `dune.com` недоступен из этой
сессии (egress заблокирован, см. выше). Проверить остаток можно только
на [dune.com/settings/billing](https://dune.com/settings/billing) вручную.

**Вероятная причина** (SQL-баг, не "просто мало кредитов"): у
`sql/01_pool_creation_blocks.sql` не было фильтра по дате на join с
`robinhood.transactions` — по чистому равенству `t.hash =
p.creation_tx_hash`, без предиката по времени, что лишает Dune
возможности partition pruning и, скорее всего, заставляет читать
таблицу транзакций всего чейна целиком. То же потенциально относится к
`sql/04_sniper_insider_exclusions.sql` (join против `robinhood.traces` и
`erc20_robinhood.evt_transfer` — тоже общечейновые таблицы). **Уже
исправлено** (см. историю коммитов) — оба запроса теперь дополнительно
фильтруют по `{{start_date}}/{{end_date}}` (июльское окно) на стороне
джойна, а не только по ключу соединения.

**Пайплайн НЕ перезапускался повторно после этого фикса** — по вашему
правилу "стоп на 402/429", решение о повторной попытке (после проверки/
пополнения баланса) — за вами.
