...
def main() -> int:
    ...
    for с in закрытые:
        ...
        строка = {
            ...
            # ТРИ ПОЛЯ ПОРОЗНЬ (слово владельца 26.09: "запись = цепь до
            # лампорта по всем трём полям -- вход, возврат, расход"):
            #   возврат  -- нативная дельта НАШЕЙ продажи;
            #   вход+расход -- нативная дельта НАШЕЙ покупки со знаком минус
            #                  (вход и расход порознь цепь не различает: это
            #                  одна и та же трата, поэтому проверяется сумма);
            #   итог     -- сумма двух дельт.
            "возврат_сошёлся": (None if с.get("closed_sol_net") is None else
                                 abs(float(с["closed_sol_net"]) - d_прод) < 1e-9),
            "вход_плюс_расход_по_записи": (
                None if (с.get("sol_in") is None
                          or с.get("pnl_counted_spend_sol") is None) else
                round(float(с["sol_in"]) + float(с["pnl_counted_spend_sol"]), 9)),
            "вход_плюс_расход_по_цепи": round(-d_куп, 9),
            "вход_расход_сошлись": (
                None if (с.get("sol_in") is None
                          or с.get("pnl_counted_spend_sol") is None) else
                abs(float(с["sol_in"]) + float(с["pnl_counted_spend_sol"]) + d_куп) < 1e-9),
            "итог_сошёлся": (None if запись is None else
                              abs(float(запись) - по_цепи) < 1e-9),
            "подпись_покупки": куп,
    ...
    свод = {
        ...
        "порог_sol": ПОРОГ_SOL,
        # Сколько сделок сошлись ДО ЛАМПОРТА по всем трём полям сразу.
        "сделок_три_поля_до_лампорта": sum(
            1 for с in строки if с.get("возврат_сошёлся") is True
            and с.get("вход_расход_сошлись") is True and с.get("итог_сошёлся") is True),
        "сделок_с_разошедшимся_полем": sum(
            1 for с in строки if False in (с.get("возврат_сошёлся"),
                                            с.get("вход_расход_сошлись"),
                                            с.get("итог_сошёлся"))),
    ...
    for с in строки[-10:]:
        print(f"{с['закрыта_utc']} {(с['минт'] or '')[:8]} группа={с['группа']} "
              ...
              f"поля(возврат/вход+расход/итог)="
              f"{с.get('возврат_сошёлся')}/{с.get('вход_расход_сошлись')}/{с.get('итог_сошёлся')}")
