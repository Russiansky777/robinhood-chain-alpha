...
def main() -> int:
    ...
    площадка = {"решений_всего": 0, "решений_buy": 0, "по_кодам": {}}
    разбор = []
    for путь in файлы(а.state_dir, "decisions.jsonl"):
        for з in строки(путь):
            ...
            if з.get("own_send_total_ms") is not None:
                ...
                # ПРОГРАММА ПУЛА ПО СТАДИЯМ. Без неё не ответить на вопрос
                # "сколько отказов кривой стало сделками": в причинах отказа
                # программа названа, а в отправленных -- нет.
                прог = (з.get("pool_program") or "нет программы")[:8]
                ключ_п = f"{ст}:{прог}"
                полоса.setdefault("программы_по_стадиям", {})
                полоса["программы_по_стадиям"][ключ_п] = (
                    полоса["программы_по_стадиям"].get(ключ_п, 0) + 1)
                ...
                continue
            # СЧИТАЕМ ВСЕ КОДЫ РЕШЕНИЙ, А НЕ ТОЛЬКО ПОКУПКИ. Прогон 14:16Z
            # показал нули там, где на самом деле были отказы: при стоящем
            # KILL_BLOOM решение становится skip с кодом KILL_SWITCH, а не
            # buy -- и счётчик "решений_buy" честно показывал ноль, пряча сам
            # отказ. Теперь видно и то, и другое.
            если_код = з.get("code")
            if если_код:
                площадка["по_кодам"][если_код] = площадка["по_кодам"].get(если_код, 0) + 1
                площадка["решений_всего"] = площадка.get("решений_всего", 0) + 1
            if з.get("action") == "buy":
                    ...
                    разбор.append({
                        ...
                        "exec": {к: v for к, v in (з.get("exec") or {}).items()
                                 if к in ("exec_code", "bloom_ms", "api_ms",
                                           ...
                                           "http_ms", "elapsed_ms")},
                        # ВСЕ ОСТАЛЬНЫЕ ПОЛЯ ЗАПИСИ -- как есть. Прогон 13:49Z
                        # не нашёл времени ответа Bloom потому, что я угадывал
                        # имя поля; гадать не надо -- печатаем запись целиком,
                        # кроме уже названного выше. Ключей кошельков в журнале
                        # решений нет.
                        "остальное": {к: v for к, v in з.items()
                                       if к not in ("exec", "logs")
