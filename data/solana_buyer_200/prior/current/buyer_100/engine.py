...
def decode_tx(r):
 ...
 for l in m.get('logMessages') or []:
  ...
  if d[:8]==PUMP_AMM_BUY and len(d)>=184:
   ...
   # минт -- ненулевой не-WSOL/USDC/USDT баланс ИМЕННО кошелька user (offset152
   # события), и только если он ОДНОЗНАЧЕН -- если у user в этой tx несколько
   # разных таких минтов (мультихоп через промежуточный не-стейбл токен),
   # честно не угадываем, оставляем m0/m1 не заполненными.
   cand_mints={b.get('mint') for b in bal.values() if b.get('owner')==user and b.get('mint') not in (WSOL,USDC,USDT)}
   mint=next(iter(cand_mints)) if len(cand_mints)==1 else None
   ...
   elif len(cand_mints)>1:
    e['status']='ambiguous_mint_multiple_nonstable_candidates_not_decoded'
   out.append(e)
