...
def run() -> int:
    ...
    # Оценка для ПОЛНОГО прогона -- пересмотрена 2026-09-05 по РЕАЛЬНОЙ
    # цене смоука (1 выходные, окно сканирования dex.trades ~3.6 дня):
    # факт 0.6207 кредита (data/credits_spent.json, execution_id
    # 01M1RR1YNT9N58GAT3MWNF8SS3). Полный прогон сканирует то же число
    # токенов, но ~59.6 дней вместо ~3.6 -- при типичном для Dune
    # спелбука масштабировании стоимости сканирования time-партиций
    # почти линейно по числу дней это даёт оценку ~0.62×16.6≈10.3
    # кредита; берём 15.0 с запасом -- ПРЕЖНЯЯ жёсткая оценка 60.0 была
    # не по факту, а "на всякий случай", и упёрлась в санитарный порог
    # >40 (run 33967832537), хотя реальная цена была бы на порядок ниже.
    df = client.run_sql_cached(
        "task1_weekend_windows", sql, query_id=qid,
        estimated_credits=8.0 if SMOKE_ONLY else 15.0,
