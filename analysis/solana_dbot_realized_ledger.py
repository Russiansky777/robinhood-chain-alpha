...
def wallet_buy_for_mint(wallet: str, mint: str, before_time: int, lookback_s: int = 300) -> dict | None:
    """Ищет покупку mint этим кошельком В ОКНЕ [before_time-lookback_s,
    before_time] -- используется для атрибуции копии к одному из 10
    отслеживаемых кошельков батча (не для точной цены -- для этого есть
    leader_buy_for_mint)."""
    ...
    for acc in accounts:
        ...
        for _ in range(5):
            ...
            if not batch:
                break
            sigs.extend(batch)
            if batch[-1].get("blockTime") and batch[-1]["blockTime"] < before_time - lookback_s:
                break
            before = batch[-1]["signature"]
            if len(batch) < 1000:
                break
        candidates = [s for s in sigs if s.get("blockTime") and before_time - lookback_s <= s["blockTime"] <= before_time
...
def find_source_wallet(tracked_wallets: list[dict], mint: str, before_time: int) -> dict | None:
    ...
    с blockTime <= before_time в окне lookback) -- это и есть источник
    копии. Если ни один не найден в окне -- честно None (не гадаем)."""
    best = None
    for tw in tracked_wallets:
        addr = tw.get("address")
        if not addr:
            continue
        r = wallet_buy_for_mint(addr, mint, before_time)
...
def leader_buy_for_mint(mint: str, before_time: int) -> dict | None:
    ata_result = fp.rpc_call("getTokenAccountsByOwner", [LEADER_WALLET, {"mint": mint}, {"encoding": "jsonParsed"}])
    accounts = (ata_result or {}).get("value", [])
    for acc in accounts:
        ata = acc["pubkey"]
        sigs, before = [], None
...
def build_trade_row(label: str, wallet: str, task_id: str, source_info: dict | None, pair: dict) -> dict:
    ...
    if leader_buy and leader_buy.get("paid_usdc") and leader_buy.get("tokens_received"):
...
def wallet_balance_reconciliation(wallet: str, all_meta: list[dict], trade_pairs: list[dict]) -> dict:
    ...
    total_open_value_sol = total_open_value_usd / sol_usd_now if sol_usd_now else None
    ...
    return {
        ...
        "total_open_value_sol_approx": total_open_value_sol,
        "implied_pnl_sol": (current_balance + total_open_value_sol - total_funded)
                           if current_balance is not None and total_open_value_sol is not None else None,
    }
