...
def pool_reserves(tx: dict, mint: str, trader: str) -> dict:
    ...
    best = None
    def качество(c) -> float:
        """Насколько кандидат ведёт себя как постоянное произведение.

        Две независимые проверки сразу: сохранилось ли k и сходится ли
        средняя цена по ногам с sqrt(цена_до * цена_после). У настоящего
        пула обе мелкие; у случайно совпавшего движения хотя бы одна
        уезжает. Только по k отбор ошибался: слабое движение сохраняет k
        само собой.
        """
        t0, t1 = c["резерв_токена_до"], c["резерв_токена_после"]
        q0, q1 = c["резерв_котировки_до"], c["резерв_котировки_после"]
        if t0 <= 0 or t1 <= 0 or q0 <= 0 or q1 <= 0:
            return 9e9
        p0, p1 = q0 / t0, q1 / t1
        dt, dq = t1 - t0, q1 - q0
        avg = abs(dq) / abs(dt) if dt else None
        модель = (p0 * p1) ** 0.5
        расх = abs(модель / avg - 1) if avg else 9e9
        return max(abs(c.get("k_изменилось_на", 9e9)), расх)

    def лучше(a, b) -> bool:
        if b is None:
            return True
        ka, kb = качество(a), качество(b)
        if abs(ka - kb) > 1e-12:
            return ka < kb
