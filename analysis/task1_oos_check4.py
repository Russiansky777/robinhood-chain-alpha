...
HIT_RATE_THRESHOLD = 0.60  # предрегистрировано владельцем ДО прогноза
...
def predict() -> int:
    ...
    predictions = x_ok[["symbol", "X", "predicted_sign_Z"]].to_dict("records")
    out = {
    ...
    print(f"[oos_check4:predict] {len(predictions)} реальных прогнозов записано в {PREDICT_PATH} (timestamp {now})")
    return 0
...
def verify() -> int:
    ...
    if hit_rate is not None:
        verdict = (f"ПРОШЛА (hit_rate={hit_rate:.1%} >= {HIT_RATE_THRESHOLD:.0%})" if hit_rate >= HIT_RATE_THRESHOLD
                   else f"НЕ ПРОШЛА (hit_rate={hit_rate:.1%} < {HIT_RATE_THRESHOLD:.0%})")
    ...
    out = {
        ...
        "n_scored": len(scored), "n_hit": n_hit, "hit_rate": hit_rate, "verdict": verdict,
        "rows": rows,
    }
    VERIFY_PATH.parent.mkdir(parents=True, exist_ok=True)
    VERIFY_PATH.write_text(json.dumps(out, indent=2, ensure_ascii=False, default=str))
    print(f"[oos_check4:verify] N_scored={len(scored)} n_hit={n_hit} hit_rate={hit_rate} verdict={verdict}")
