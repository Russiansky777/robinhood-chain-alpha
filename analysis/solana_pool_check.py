...
def check_side(pool: dict | None, fact_price: float | None, label: str) -> dict:
    """Сверка одной стороны: модель sqrt(до*после) против чека трейдера."""
    out = {"сторона": label}
    ...
    out["допуск"] = tol
    if not fact_price or fact_price <= 0:
        out.update(прошла=None, почему="нет фактической средней цены исполнения")
        return out
    dev = abs(model / fact_price - 1)
    out["отклонение"] = round(dev, 6)
    out["прошла"] = dev <= tol
    if not out["прошла"]:
        out["почему"] = (f"сверка не прошла: модель {model:.12g}, факт {fact_price:.12g}, "
                          f"расхождение {dev * 100:.2f}% при допуске {tol * 100:.0f}%")
    return out
...
def summarise(label: str, items: list[dict]) -> dict:
    ...
    return {
        ...
        "медиана_резерва_до_лидера": _med([x.get("резерв_котировки_до_лидера") for x in good]),
...
def self_test() -> None:
    ...
    # Идеальный пул x*y=k: модель обязана сойтись с фактом ровно.
    pool = {"ок": True, "цена_до": 0.001, "цена_после": 0.001 * 1.21, "программа": "Raydium"}
    model = math.sqrt(pool["цена_до"] * pool["цена_после"])
    r = check_side(pool, model, "лидер")
    chk("идеальная сходимость проходит", r["прошла"] is True, str(r.get("отклонение")))
    r2 = check_side(pool, model * 1.02, "лидер")
    chk("2% при допуске 3% проходит", r2["прошла"] is True, str(r2.get("отклонение")))
    r3 = check_side(pool, model * 1.10, "лидер")
