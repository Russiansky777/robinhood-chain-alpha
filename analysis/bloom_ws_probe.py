...
def найти_путь(о, путь: str):
    """Значение по точечному пути, с поддержкой обеих форм уведомления."""
    текущий = о
...
class Замер:
    ...
    def учесть_tx(self, сообщение: dict, источник: str | None) -> None:
        ...
        for поле in ВАЖНЫЕ_ПОЛЯ:
            есть = найти_путь(res, поле) is not None
            c = self.поля_счёт.setdefault(поле, {"есть": 0, "нет": 0})
            c["есть" if есть else "нет"] += 1
        if len(self.образцы) < ОБРАЗЦОВ:
            self.образцы.append({"подпись": sig, "слот": слот,
                                  "форма": форма(res)})
        if sig:
            self.tx.setdefault(sig, {"t": time.time(), "слот": слот,
                                      "источник": источник})
    ...
    def учесть_logs(self, сообщение: dict, источник: str | None) -> None:
        ...
        if isinstance(sig, str) and BD.SIG_RE.match(sig):
            self.logs.setdefault(sig, {"t": time.time(), "слот": слот,
                                        "источник": источник})
...
def разобрать_расхождение(helius, подписи: list, источники: dict,
    ...
    for sig in подписи[:предел]:
        ...
        (итог["подписано_источником"] if свои else итог["только_упомянут"]).append(
            {"подпись": sig, "подписанты_из_источников": свои})
...
async def прогон(секунд: float, задачи: tuple, снимок: Path) -> dict:
    ...
    ключевые = ("transaction.meta.preTokenBalances",
                 ...
                 "transaction.meta.preBalances",
                 "transaction.meta.postBalances")
    можно_разбирать = bool(замер.сообщений_tx) and all(
        (достаточно[k]["доля"] or 0) >= 0.99 for k in ключевые)

    return {
        "окно_с": секунд,
        "снято_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "источников": len(адреса),
        "источники_откуда": откуда,
        ...
        "опережение_logs_над_tx_мс": {
            "n": len(опережение),
            "медиана": опережение[len(опережение) // 2] if опережение else None,
            "мин": опережение[0] if опережение else None,
            "макс": опережение[-1] if опережение else None},
...
def self_test() -> int:
    ...
    о = {"transaction": {"meta": {"preTokenBalances": []}}}
    chk("путь находит пустой список", найти_путь(о, "transaction.meta.preTokenBalances") == [])
    chk("несуществующий путь -- None", найти_путь(о, "transaction.meta.нет") is None)
    chk("путь сквозь не-словарь -- None", найти_путь({"a": 5}, "a.b") is None)
