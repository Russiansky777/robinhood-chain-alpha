...
def симулировать(уз: Узел, покупка: dict, *, опора: str | None = None,
    ...
    if опора is None:
        опора = подпись_после_слота(уз, макс_слот + 1)
    ...
    окно_полное = опора is not None
    из_["окно_полное"] = окно_полное
...
def скан_кошелька(уз: Узел, кош: str, с_ts: float, до_ts: float, *,
                  ...
                  предел_подписей: int | None = None) -> dict:
