...
class Пакетный(P.Узел):
    """Тот же узел, плюс ПАКЕТНЫЙ запрос транзакций и отступ на 429.

    ЗАЧЕМ ПАКЕТ. Первый прогон п.1б (26.09 12:41Z) уперся в предел вызовов на
    2491 транзакции из 6585 подписей лидера: по одному вызову на транзакцию
    окно не закрывается, а неполный список врёт про "источник купил, а мы нет".
    JSON-RPC разрешает массив запросов в одном POST -- значит те же 6585
    транзакций стоят 132 обращения к узлу, а не 6585. Кредиты узел всё равно
    считает по запросам внутри пакета, и это число в своде отдельно.

    ЗАЧЕМ ОТСТУП. В том же прогоне 195 вызовов вернулись ошибкой: без отступа
    при 429 прогон продолжает стучаться и получает ещё больше отказов.
    """
    ...
    def __init__(self) -> None:
        super().__init__()
        self.обращений = 0   # POST к узлу
        self.запросов = 0    # запросов внутри пакетов -- по ним считаются кредиты
        self.отказов_429 = 0
    ...
    def пакет_транзакций(self, подписи: list) -> dict:
        """{подпись: tx или None}. Найденное кладётся в общий кэш."""
        из_ = {}
        осталось = [п for п in подписи if п not in self._кэш]
        for п in подписи:
            if п in self._кэш:
                из_[п] = self._кэш[п]
        for и in range(0, len(осталось), self.РАЗМЕР_ПАКЕТА):
            кусок = осталось[и:и + self.РАЗМЕР_ПАКЕТА]
            ...
            по_id = {}
            for о in (ответ if isinstance(ответ, list) else [ответ]):
...
def план_отдаёт_транзакции(уз: P.Узел, адрес: str) -> dict:
    """Проверка ПЕРВЫМИ вызовами: отдаёт ли план getTransactionsForAddress.

    Требование владельца по заданию 2. Пробуются ТРИ вида параметров, потому что
    первый прогон получил -32602 "Expected end of params" -- то есть метод
    существует, но второй параметр ему не нравится, и объявлять "плана нет" по
    одной форме было бы неправдой. Ни один не сработал -- идём длинным путём
    (подписи плюс пакетные getTransaction) и говорим об этом в своде.
    """
    формы = [
        ("адрес и словарь", [адрес, {"limit": 1, "transactionDetails": "full",
                                      "status": "succeeded",
                                      "maxSupportedTransactionVersion": 0}]),
        ("один словарь", [{"address": адрес, "limit": 1,
                            "transactionDetails": "full", "status": "succeeded"}]),
        ("только адрес", [адрес]),
    ]
    попытки = []
    for имя, парам in формы:
        try:
            рез = уз.вызов("getTransactionsForAddress", парам)
        except Exception as exc:  # noqa: BLE001
            попытки.append({"форма": имя, "ok": False,
                             "why_not": f"{type(exc).__name__}: {str(exc)[:140]}"})
            continue
        if isinstance(рез, (list, dict)) and рез:
            return {"ok": True, "форма": имя, "вид_ответа": type(рез).__name__,
                    "попытки": попытки}
        попытки.append({"форма": имя, "ok": False,
                         "why_not": f"пустой ответ вида {type(рез).__name__}"})
    return {"ok": False, "why_not": "ни одна форма параметров не отдала транзакции",
            "попытки": попытки}
...
def покупки_источника(уз: Пакетный, адрес: str, подписи: list, *,
                      предел_вызовов: int) -> dict:
    """Из подписей окна оставить ПОКУПКИ: токен пришёл, SOL ушёл.

    Транзакции берутся ПАКЕТАМИ: по одной на вызов окно лидера не закрывается.
    """
    из_ = {"покупки": [], "смотрено": 0, "предел_уперся": False,
            "не_покупки": 0, "отказов": 0}
    по_подписи = {з["signature"]: з for з in подписи}
    очередь = [з["signature"] for з in подписи]
    for и in range(0, len(очередь), уз.РАЗМЕР_ПАКЕТА):
        if уз.вызовов + уз.РАЗМЕР_ПАКЕТА > предел_вызовов:
            из_["предел_уперся"] = True
            break
        кусок = очередь[и:и + уз.РАЗМЕР_ПАКЕТА]
        try:
            txs = уз.пакет_транзакций(кусок)
        except Exception:  # noqa: BLE001
            из_["отказов"] += len(кусок)
            continue
        for подпись in кусок:
            tx = txs.get(подпись)
            из_["смотрено"] += 1
            if not tx:
                из_["отказов"] += 1
...
def main() -> int:
    ...
    уз = Пакетный()
    начало = time.time()
    # ПРЕДЕЛ ДЕЛИТСЯ НА ИСТОЧНИКИ, И НА ЧИСЛА ОСТАЁТСЯ ЗАПАС. В первом прогоне
    # предел был общий, лидер съел его весь, и Brez остался с нулём подписей --
    # это выглядело как "у Brez покупок нет", хотя его просто не смотрели.
    ЗАПАС_НА_ЧИСЛА = 300

    журнал = записи_журнала(Path(а.ledger), адреса, с_ts, до_ts)
    план = план_отдаёт_транзакции(уз, next(iter(адреса)))
    if план.get("ok") and план.get("форма") != "адрес и словарь":
        план["листается"] = False
        план["почему_не_годится"] = ("метод отвечает только на один параметр-адрес: "
                                      "ни окна, ни страниц задать нельзя")
    elif план.get("ok"):
        план["листается"] = True

    по_источнику = {}
    доля = max(400, (а.predel_vyzovov - ЗАПАС_НА_ЧИСЛА - уз.вызовов) // max(1, len(адреса)))
    for адрес, имя in адреса.items():
        предел_этого = min(а.predel_vyzovov - ЗАПАС_НА_ЧИСЛА, уз.вызовов + доля)
        зап = [т for т in журнал if т.get("source_address") == адрес]
        по_минту: dict = {}
        for т in зап:
            по_минту.setdefault(т.get("mint"), []).append(т)
        # ПЛАН ГОДИТСЯ ТОЛЬКО ЕСЛИ ЕГО МОЖНО ЛИСТАТЬ. Прогон 12:55Z: форма
        # «только адрес» отвечает (dict), но второй параметр метод не берёт
        # вовсе -- значит ни окна, ни страниц задать нельзя, и такой ответ для
        # восьми дней бесполезен. Идём длинным путём с пакетами и говорим об
        # этом словами, а не выдаём первую страницу за всё окно.
        if план.get("ok") and план.get("форма") == "адрес и словарь":
            пк = покупки_через_план(уз, адрес, с_ts, до_ts,
                                     предел_вызовов=предел_этого)
            сп = {"подписи": [], "страниц": пк["страниц"],
                  "предел_уперся": пк["предел_уперся"], "why_not": пк["why_not"],
                  "путь": "getTransactionsForAddress"}
        else:
            сп = подписи_окна(уз, адрес, с_ts, до_ts, предел_вызовов=предел_этого)
            сп["путь"] = "getSignaturesForAddress + пакетные getTransaction"
            пк = покупки_источника(уз, адрес, сп["подписи"], предел_вызовов=предел_этого)
        совпало, без_нас = [], []
        занятые = set()
        for п in пк["покупки"]:
        ...
        по_источнику[имя] = {
            ...
            "покрытие_окна": (round(пк["смотрено"] / len(сп["подписи"]), 4)
                              if сп["подписи"] else None),
            "узел_не_отдал": пк.get("отказов"),
    ...
    итог = {
        ...
        "обращений_к_узлу": уз.обращений, "запросов_в_пакетах": уз.запросов,
        "отказов_429": уз.отказов_429,
