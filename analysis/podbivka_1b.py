...
def чисто(текст) -> str:
    """Вычистить из строки адрес узла и ключ.

    ЗАЧЕМ. Прогон 13:0xZ записал в отчёт текст ошибки requests целиком, а в нём
    был полный URL узла с api-key -- и файл с ключом уехал в репозиторий. Любая
    строка ошибки теперь проходит через эту вычистку ПЕРЕД тем, как попасть в
    отчёт или в печать.
    """
    т = str(текст)
...
def не_последняя(попытка: int, всего: int) -> bool:
    return попытка < всего - 1
...
class Пакетный(P.Узел):
    ...
    # 50 запросов в пакете давали 56 отказов 429 и 540 не отданных транзакций
    # за прогон. 25 плюс короткая пауза между пакетами -- та же работа без
    # потерь, а потерянная транзакция это дыра в списке покупок источника.
    РАЗМЕР_ПАКЕТА = 25
    ПАУЗА_МЕЖДУ_ПАКЕТАМИ_С = 0.15
    ...
    def вызов(self, метод: str, парам: list, *, срок: float = 25.0):
        """Одиночный вызов с отступом на 429.

        ЗАЧЕМ. Прогон 13:0xZ: список подписей Brez не собрался вовсе из-за
        ОДНОГО 429 на getSignaturesForAddress -- и в отчёте это выглядело как
        "покупок у Brez нет". Одиночные вызовы теперь тоже ждут и пробуют снова.
        """
        for попытка in range(5):
            try:
                return super().вызов(метод, парам, срок=срок)
            except Exception as exc:  # noqa: BLE001
                если429 = "429" in str(exc) or "Too Many" in str(exc)
                if не_последняя(попытка, 5) and если429:
                    self.отказов_429 += 1
    ...
    def пакет_транзакций(self, подписи: list) -> dict:
        ...
        for и in range(0, len(осталось), self.РАЗМЕР_ПАКЕТА):
            ...
            for j, п in enumerate(кусок):
                о = по_id.get(j) or {}
                if "error" in о or not о:
                    пропали.append(п)
                    continue
                tx = о.get("result")
                self._кэш[п] = tx
                из_[п] = tx
            # ПРОПАВШИЕ ПЕРЕСПРАШИВАЕМ ПО ОДНОЙ. Пропуск -- это дыра в списке
            # покупок источника, а дыра врёт про "источник купил, а мы нет".
            for п in пропали:
                try:
                    tx = self.вызов("getTransaction", [п, P.ОПЦИИ_TX])
    ...
    for имя, парам in формы:
        try:
            ...
            попытки.append({"форма": имя, "ok": False,
                             "why_not": чисто(f"{type(exc).__name__}: {exc}")[:200]})
            continue
...
def подписи_окна(уз: P.Узел, адрес: str, с_ts: float, до_ts: float, *,
    ...
    while True:
        ...
        try:
            ...
            из_["why_not"] = чисто(f"{type(exc).__name__}: {exc}")[:200]
            break
        из_["страниц"] += 1
        if not стр:
            break
...
def сверить_числа(пары: list, уз: P.Узел, *, предел_вызовов: int) -> dict:
    ...
    for п in пары:
        ...
        try:
            ...
            строка["why_not"] = чисто(f"узел: {type(exc).__name__}")
            отказы.append(строка)
            continue
...
def main() -> int:
    ...
    for адрес, имя in адреса.items():
        ...
        по_источнику[имя] = {
            ...
            # ПОКРЫТИЕ -- ПО РАЗОБРАННЫМ, А НЕ ПО ПОПЫТКАМ. В прогоне 13:0xZ
            # покрытие показывало 1.0 при 540 не отданных узлом транзакциях:
            # это была неправда о полноте списка.
            "покрытие_окна": (round((пк["смотрено"] - пк.get("отказов", 0))
                                     / len(сп["подписи"]), 4)
