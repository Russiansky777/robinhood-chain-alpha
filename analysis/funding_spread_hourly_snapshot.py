...
Источники (оба -- реальные прямые факты, не производные):
  - Lighter: `/api/v1/fundings` (market_id, resolution=1h,
    ...
    доказательства в docs/P4_RECON.md ("дозапрос: единицы фандинга"),
    ПЛЮС четвёртое, прямое: 2026-09-04 сверено с РЕАЛЬНЫМ Δfunding на
    живой позиции 1000756 (analysis/verify_lighter_rate_units.py,
    data/p3_guard_cache/verify_lighter_rate_units_result.json) -- три
    последовательные пары точек data/p5_fee_accrual.jsonl дали
    ratio_if_rate_is_pct = 0.84 / 0.54 / 1.00 (порядок 1, разброс --
    шум от несовпадения границ снимков с часовыми окнами Lighter), тогда
