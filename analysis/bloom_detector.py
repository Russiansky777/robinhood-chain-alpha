        ...
        try:
            for к, п in (self.состояние.positions() or {}).items():
                ...
                if подпись and подпись == п.get("control_signature"):
                    ...
                    return зк
                # ВЕЕР ПО СЕРВИСАМ: у каждого отправителя своя подпись. Без
                # этой ветки они считались бы покупкой, и замер покупки был бы
                # затёрт замером пустышки -- ровно то, чего мы избегаем выше.
                свой = None
...
async def слушать(детектор: Детектор, ключ: str, *, стоп_через_s: float | None = None) -> None:
    ...
    while True:
        ...
        try:
            ...
            if atlas:
                log.warning("перехожу на запасной logsSubscribe не дольше %.0f с",
                            ОКНО_ЗАПАСНОГО_S)
                atlas = False
...
# путь. Две минуты: обрыв основного обычно лечится переподключением, а жить
# на запасном дорого и медленно -- он не несёт транзакцию, и каждый сигнал
# стоит getTransaction на commitment confirmed.
ОКНО_ЗАПАСНОГО_S = ST.env_float("BLOOM_FALLBACK_WINDOW_S", 120.0)
...
def self_test() -> int:
    ...
    with _врем_каталог() as d:
        ...
        try:
            ...
            chk("веер НЕ затирает ни покупку, ни основной контроль",
                abs(поз_в.get("lane_send_to_seen_ms") - 300.0) < 5.0
                and abs(поз_в.get("control_send_to_seen_ms") - 140.0) < 5.0,
                (поз_в.get("lane_send_to_seen_ms"),
                 поз_в.get("control_send_to_seen_ms")))
    ...
    with _врем_каталог() as врем_сл:
        ...
        д_сл = Детектор(источники={"SRCL": "BATCH-5"}, состояние=st_сл,
                         курс=КурсSOL(), режим="dry", helius=HeliusДляСлушания())
        было_ws = globals().get("websockets")
        было_окно = globals()["ОКНО_ЗАПАСНОГО_S"]
        было_тик = globals()["ТИК_ПОДПИСКИ_S"]
        globals()["websockets"] = МодульWS
        globals()["ОКНО_ЗАПАСНОГО_S"] = 0.2
        globals()["ТИК_ПОДПИСКИ_S"] = 0.05
        try:
            _aio.run(слушать(д_сл, "КЛЮЧ", стоп_через_s=2.6))
        except Exception as exc:  # noqa: BLE001
            подписки_сделаны.append(f"упало: {type(exc).__name__}: {exc}")
        finally:
            globals()["websockets"] = было_ws
            globals()["ОКНО_ЗАПАСНОГО_S"] = было_окно
            globals()["ТИК_ПОДПИСКИ_S"] = было_тик
        for з_ in подписки_сделаны:
            if з_ == "--соединение--":
                методы_соединений.append(None)
            elif методы_соединений and методы_соединений[-1] is None:
                методы_соединений[-1] = з_
        chk("после обрыва идём на запасной, а из тишины на запасном ВОЗВРАЩАЕМСЯ",
            методы_соединений[:3] == ["transactionSubscribe", "logsSubscribe",
                                       "transactionSubscribe"],
            (методы_соединений, подписки_сделаны[:8]))
        chk("тишина считается тиками, а не обрывами",
            д_сл.тиков_тишины > 0 and д_сл.обрывов == 1,
            (д_сл.тиков_тишины, д_сл.обрывов))
        chk("обрыв назван причиной, а не потерян",
            any("соединение закрыто" in п or "незнакомая" in п
                for п in д_сл.обрывов_по_причинам), д_сл.обрывов_по_причинам)
        chk("время на запасном посчитано и эпизод закрыт",
            д_сл.запасное_время_с > 0 and д_сл.запасной_путь_с is None,
            (д_сл.запасное_время_с, д_сл.запасной_путь_с))
