...
def адреса_подписки(детектор: "Детектор") -> list:
    ...
    """
    return sorted(set(детектор.источники) | {ST.EXECUTOR_WALLET}
                   | set(детектор.кэш_ног_адреса))
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        chk("четыре хранилища котировок стоят в списке подписки",
        ...
        chk("первое событие кэша легло в журнал на сверку с ingest_stats",
            ...
            and детектор_к.ног_первые[0]["quote"] is not None,
        ...
        chk("признак жизни показывает кэш словами и числами",
            ж_к.get("enabled") and ж_к.get("pools") == 4
            and ж_к.get("fed") == 1 and ж_к.get("no_tx") == 1
            and isinstance(ж_к.get("ingest_stats"), dict), ж_к)
