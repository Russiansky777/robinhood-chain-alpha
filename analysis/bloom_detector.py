...
class Детектор:
    ...
    def тревога_ухода_на_запасной(self) -> None:
        ...
        self.сказать_о_запасном(NT.строка_тревоги(
            "детектор ушёл на запасной logsSubscribe",
            f"возврат через {ОКНО_ЗАПАСНОГО_S:.0f} с; тревога, если не вернётся"))
    ...
    def проверить_запасной_путь(self, *, сейчас: float | None = None) -> float | None:
        ...
        if NT is not None:
            self.сказать_о_запасном(NT.строка_тревоги(
                f"детектор на запасном пути {сколько:.0f} с",
                "уведомления без транзакции: каждый сигнал стоит getTransaction"))
        return сколько
    ...
    def сообщить_пару(self, cid_полосы: str) -> dict:
        ...
        текст = (f"🏁 пара «Bloom против нашей» по {(источник or '')[:12]}\n"
                  ...
                  f"{отставание(наша)}\n"
                  f"  от сигнала {нет(наша.get('own_tx_seen_ms'), ' мс')}, "
                  ...
                  f"{отставание(блум)}\n"
                  f"  от сигнала {нет(блум.get('own_tx_seen_ms'), ' мс')}, "
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            детектор_ч.тень_вызовов = 7
            детектор_ч._записать_кредиты_тени()
            детектор_ч2 = Детектор(источники={"SRC": "BATCH-5"}, состояние=st,
                                    курс=КурсSOL(), режим="dry",
                                    helius=HeliusДляТени())
        ...
        try:
            ...
            chk("запуск полосы посчитан отдельно от пути",
                ...
                and детектор_пп.полос_потоков_живых() == 0,
        ...
        chk("на пульсе тревога уходит строкой владельцу",
            д_зп.проверить_запасной_путь(сейчас=1200.0) == 200.0
            and len(сказанное) == 1
            and "запасном пути 200" in сказанное[0], сказанное)
        chk("строка тревоги начинается с предупреждения",
            сказанное and сказанное[0].startswith("\u26a0"), сказанное[:1])
        chk("повторная проверка сразу молчит",
            д_зп.проверить_запасной_путь(сейчас=1205.0) is None
            and len(сказанное) == 1)
        д_зп.тревога_возврата_на_основной(210.0)
        chk("возврат на основной путь тоже строка",
            len(сказанное) == 2 and "вернулся на основной" in сказанное[1], сказанное)
        д_зп.тревога_ухода_на_запасной()
        chk("уход на запасной тоже строка",
            len(сказанное) == 3 and "ушёл на запасной" in сказанное[2], сказанное)
        ...
        chk("тревоги идут в ОСНОВНОЙ чат, а не в журнал",
            адресаты and set(адресаты) == {"main"}, адресаты)
