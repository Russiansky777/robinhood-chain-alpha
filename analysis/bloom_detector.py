...
def полосу_пускать(строка: dict) -> bool:
    ...
    ТРИ РУБИЛЬНИКА -- ТРИ ПОВЕДЕНИЯ (слово владельца 26.09):
      общий KILL          -- не идёт никто: решение становится skip без метки;
      KILL_BLOOM          -- площадка отказывает своим кодом, полоса ИДЁТ:
                             решение помечено lane_allowed;
      KILL_OWN_SEND       -- решение остаётся покупкой (Bloom покупает), а
                             полосу останавливает её собственный гейт внутри
                             bloom_own_send -- сюда он не заходит вовсе.
    Функция отдельная и чистая, потому что раньше это условие жило одной
    строкой в горячем пути и проверить его было нечем: замер окна KILL_BLOOM
    26.09 14:05:50-14:16:00Z дал отказов площадки 7 и покупок полосы 0.
    """
    з = строка or {}
    if з.get("action") == "buy":
        return True
    return bool(з.get("lane_allowed"))
...
def решение(сигнал: dict, *, состояние, трата_sol: float | None,
    ...
    if not можно:
        строка.update({"action": "skip", "code": код2, "reason": почему,
                        "filter": "наш лимит",
                        "dbot_бы_купил": True})
        # ОТКАЗ ТОЛЬКО ПЛОЩАДКИ НЕ ЗАКРЫВАЕТ ПОЛОСУ. Полоса запускается по
        # решению, а решение при рубильнике площадки становилось skip -- и
        # полоса не шла вовсе (замер 26.09: отказов площадки 7, покупок полосы
        # 0 за десять минут). Метка говорит вызывающему: Bloom не идёт, полоса
        # идёт, и её собственные гейты (KILL_OWN_SEND, потолок, предел
        # открытых) решают сами.
        if код2 == ST.КОД_РУБИЛЬНИК_BLOOM:
            строка["lane_allowed"] = True
        return строка
...
class Детектор:
    ...
    def признак_жизни(self) -> dict:
        ...
        # ПОЗИЦИИ ЧИТАЮТСЯ ОДИН РАЗ НА ПРИЗНАК ЖИЗНИ. Их чтение -- это чтение
        # журнала с диска, и второй вызов на тот же тик стоил бы столько же.
        поз_полосы = {}
        if OS is not None and self.полоса_включена:
            try:
                поз_полосы = self.состояние.positions()
            except Exception as exc:  # noqa: BLE001
                log.warning("позиции для признака жизни не прочитаны: %s", type(exc).__name__)
        st["own_send"] = {
            ...
            "lane_state": (OS.состояние_полосы(поз_полосы)
                            if OS is not None and self.полоса_включена else {}),
            # ПОЛОСА ПО ВСЕМ ГРУППАМ (I.5). Группа скорости считается отдельно
            # по слову владельца: у неё свой потолок и свой стоп. Раньше в
            # признаке жизни был виден только счёт "денежных" групп, и где
            # стоит speed_only -- не видел никто.
            "lane_state_by_group": ({
                "money": OS.состояние_полосы(поз_полосы),
                OS.ГРУППА_СКОРОСТИ: OS.состояние_полосы(поз_полосы, группа=OS.ГРУППА_СКОРОСТИ),
            } if OS is not None and self.полоса_включена else {}),
    ...
    def обработать(self, подпись: str, слот: int | None, источник: str | None,
        ...
        if полосу_пускать(строка) and self.исполнитель is not None:
            ...
            # Bloom его не ждёт.
            # lane_allowed -- отказ, который касается ТОЛЬКО площадки
            # (рубильник KILL_BLOOM). Общий KILL такой метки не ставит, и тогда
            # не идёт никто.
...
def self_test() -> int:
    ...
    with _врем_каталог() as d:
        ...
        # становилось skip, и полоса не шла ни при одном.
        # (б) РУБИЛЬНИК ТОЛЬКО ПЛОЩАДКИ: свой код, метка полосе, полоса идёт.
        st.kill_bloom_path.parent.mkdir(parents=True, exist_ok=True)
        st.kill_bloom_path.write_text("стоп только Bloom", encoding="utf-8")
        rб = решение(s, состояние=st, трата_sol=2.5, баланс_sol=3.0, текущий_слот=101)
        chk("KILL_BLOOM: площадка отказывает СВОИМ кодом",
            rб["action"] == "skip" and rб["code"] == ST.КОД_РУБИЛЬНИК_BLOOM, rб)
        chk("и полоса по этому решению ИДЁТ",
            rб.get("lane_allowed") is True and полосу_пускать(rб) is True, rб)
        st.kill_bloom_path.unlink()

        # (в) РУБИЛЬНИК ТОЛЬКО ПОЛОСЫ: решение остаётся покупкой -- Bloom
        # покупает, а полосу останавливает её собственный гейт внутри
        # bloom_own_send, до которого это решение даже не доходит. Состояние
        # берётся ЧИСТОЕ: в st выше уже лежит открытая позиция по этому минту, и
        # решение упиралось бы в дубль, а не в рубильник.
        st_л = ST.ExecState(base=Path(d) / "s_lane", kill=Path(d) / "kill_lane_none")
        st_л.kill_lane_path.parent.mkdir(parents=True, exist_ok=True)
        st_л.kill_lane_path.write_text("стоп полосе", encoding="utf-8")
        rл = решение(s, состояние=st_л, трата_sol=2.5, баланс_sol=3.0, текущий_слот=101)
        chk("KILL_OWN_SEND: решение остаётся покупкой -- Bloom покупает",
            rл["action"] == "buy" and rл["code"] == КОД_КУПИТЬ, rл)
        chk("и рубильник полосы виден отдельно от общего",
            st_л.lane_kill_active()[0] is True and st_л.kill_active()[0] is False,
            (st_л.lane_kill_active(), st_л.kill_active()))

        # (а) ОБЩИЙ рубильник сильнее всего остального: не идёт никто.
        st.kill_path.write_text("стоп", encoding="утф-8".replace("утф", "utf"))
        r5 = решение(s, состояние=st, трата_sol=2.5, баланс_sol=3.0, текущий_слот=101)
        chk("рубильник запрещает покупку", r5["action"] == "skip", r5)
        chk("общий рубильник НЕ пускает и полосу: метки нет",
            r5.get("lane_allowed") is None and полосу_пускать(r5) is False, r5)
    ...
    with _врем_каталог() as d:
        ...
        try:
            ...
            chk("в признаке жизни у полосы стадии, пределы и возраст хеша",
                ...
                жив.get("own_send"))
            # I.5: полоса видна ПО ВСЕМ ГРУППАМ, а не только по денежным.
            по_группам = жив["own_send"].get("lane_state_by_group") or {}
            chk("в признаке жизни полоса по всем группам: деньги и группа скорости",
                set(по_группам) == {"money", "speed_only"}
                and all(isinstance(v, dict) for v in по_группам.values()),
                по_группам)
        finally:
            if было_имя_OS:
                globals()["OS"] = было_OS
