...
def вычистить(текст, ключи) -> str:
    ...
    Работа делается ОДНОЙ функцией -- bloom_senders.вычистить_секреты: две
    вычистки в двух модулях когда-нибудь разойдутся молча, и разойдутся они
    именно там, где на кону ключ. Здесь только передаём известный ключ, чтобы
    вычистка не зависела от того, что сейчас в окружении.
    """
    return SND.вычистить_секреты(текст, ключи=list(ключи or []))
