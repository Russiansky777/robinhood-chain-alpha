...
def тень(решения, с_utc: str = "") -> dict:
    ...
    for з in решения:
        ...
        # ПРАВИЛО ТО ЖЕ, ЧТО В ДЕТЕКТОРЕ (одно написание на репозиторий): если
        # в записи есть тело транзакции -- собрана это ok и тело; тела в журнале
        # обычно нет (его не пишут, чтобы журнал не рос), и тогда признак
        # сборки -- наличие приговора симуляции. Без этой оговорки счётчик
        # показывал ноль на 41 977 строках, где симуляция отвечала.
        собрана = (bool(з.get("ok")) and bool(з.get("tx_base64"))
                    if "tx_base64" in з else bool(з.get("sim_verdict")))
