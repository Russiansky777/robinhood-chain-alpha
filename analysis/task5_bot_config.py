...
# Получить точный список стоило бы ещё одного self-join-запроса
# (~230-280 кредитов) -- бюджет Mozila сейчас ~6.4 кредита, физически
# нет денег на такой запрос сегодня.
#
# Решение (реализовано ниже, БЕЗ Dune): бот САМ строит вселенную
# канонических v3-пулов на старте прямым сканированием событий
# PoolCreated через публичный RPC (0 кредитов Dune, тот же класс
# метода, что forensics в этом проекте уже использовала -- прямой
# eth_getLogs). Дальше вселенная естественно сужается порогом входа
# ($10+ ожидаемого захвата) -- пулы без достаточного расхождения просто
# никогда не сработают. Если/когда бюджет Dune позволит -- можно
# добавить отдельный офлайн-фильтр по уже накопленным адресам из нового
# дешёвого запроса, это НЕ блокирует написание и dry-run бота сегодня.
