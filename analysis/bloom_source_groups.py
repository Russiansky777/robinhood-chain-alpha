...
разница между ними -- ДЕНЬГИ, а не пометка в отчёте:
  ...
  bloom_lane  -- BATCH-3/5 плюс лидер. Bloom торгует 0.05 SOL (с 26.09, пункт 9
                 дневного плана; было 0.2), полоса 0.05 SOL.
...
ПОЛИТИКА_ПО_УМОЛЧАНИЮ = {"lane_sol": None, "bloom_trades": True, "fanout": True,
                          # ТОРГУЕТ ЛИ ПОЛОСА ПО ЭТОЙ ГРУППЕ. Отсутствие поля
                          # значит "да": так вели себя все группы до 26.09.
                          # Ноль размера и ноль потолка группу НЕ выключают
                          # (в размер_sol и потолок_группы ноль ложен и уходит
                          # в значение по умолчанию), поэтому ночное "speed_only
                          # выключен" выражается этим признаком, а не нулями.
                          "lane_trades": True,
...
def загрузить(путь: str | None = None, *, заново: bool = False) -> dict:
    ...
    for имя, г in (д.get("groups") or {}).items():
        ...
        из_["политики"][имя] = {
            ...
            # Поля нет -- полоса торгует. Явный False выключает её по группе.
            "lane_trades": (True if г.get("lane_trades") is None
                            else bool(г.get("lane_trades"))),
...
def self_test() -> int:
    ...
    chk("и порог входа у группы скорости 0.5 SOL -- один на Bloom и полосу",
        ...
        политика("speed_only"))
    # РЕШЕНИЕ ВЛАДЕЛЬЦА 26.09 (пункт 9): по bloom_lane Bloom берёт 0.05, а не
    # общий 0.2 из окружения. Раньше здесь проверялось обратное -- что поля в
    # файле нет; поле появилось, и проверка стала проверять размер.
    chk("у bloom_lane размер Bloom 0.05 из файла (было 0.2 из окружения)",
        политика("bloom_lane").get("bloom_sol") == 0.05, политика("bloom_lane"))
    ...
    chk("у скорости свой потолок 3 SOL и свой стоп -0.5",
        политика("speed_only")["day_cap_sol"] == 3.0
    ...
    # ГРУППА КАНДИДАТОВ (решение владельца 26.09). Числа проверяются здесь, а
    # не на глаз: по этой группе Bloom НЕ торгует, размер полосы 0.01, порог
    # источника 2 SOL, потолок оборота 1 SOL, стоп -0.2. Адресов пока нет -- и
    # это тоже проверяется: пустая группа ничего не меняет.
    к = политика("candidates")
    chk("у кандидатов полоса 0.01, Bloom не торгует, порог 2 SOL",
        к["lane_sol"] == 0.01 and к["bloom_trades"] is False
        and к["min_target_sol"] == 2.0, к)
    chk("и свой потолок 1 SOL со стопом -0.2, без веера",
        к["day_cap_sol"] == 1.0 and к["stop_loss_sol"] == 0.2
        and к["fanout"] is False, к)
    chk("адресов у кандидатов пока нет -- группа ничего не меняет",
        len([а for а, г in адреса_всех_групп().items() if г == "candidates"]) == 0,
        len([а for а, г in адреса_всех_групп().items() if г == "candidates"]))
