...
def составы_запусков(путь: Path) -> dict:
    """Состав каждого запуска из строк "старт" журнала: адреса по группам.

    Признак жизни помнит только ПОСЛЕДНИЙ состав, а журнал живёт дольше:
    без этих строк проверка брала текущий разгонный и на записях прошлого
    запуска честно не находила ни одного нашего адреса -- то есть врала.
    """
    источники: set = set()
    разгонные: set = set()
    стартов = 0
    with open(путь, encoding="utf-8", errors="replace") as f:
        for строка in f:
            строка = строка.strip()
            # Отбор по КЛЮЧУ "record" -- он латинский. Отбирать по значению
            # "старт" нельзя: json.dumps по умолчанию экранирует кириллицу в
            # \uXXXX, и строка в файле выглядит иначе, чем в коде.
            if not строка or '"record"' not in строка:
                continue
            try:
                з = json.loads(строка)
            except ValueError:
                continue
            if з.get("record") != "старт":
                continue
            стартов += 1
            источники.update(з.get("sources") or [])
            разгонные.update(з.get("boosters") or [])
    return {"starts": стартов, "sources": источники, "boosters": разгонные}
...
def self_test() -> None:
    ...
    ж2 = с_каталог / "feed2.jsonl"
    ж2.write_text("\n".join([
        json.dumps({"record": "старт", "utc": "T1", "sources": ["И1", "И2"],
                     "boosters": ["Р_СТАРЫЙ"], "channels": ["grpc"]}),
        json.dumps({"channel": "grpc", "signature": "A", "filter": "src0"}),
        json.dumps({"record": "старт", "utc": "T2", "sources": ["И1"],
                     "boosters": ["Р_НОВЫЙ"], "channels": ["grpc"]}),
    ]) + "\n", encoding="utf-8")
    с_сост = составы_запусков(ж2)
    chk("составы всех запусков собраны вместе",
        с_сост["starts"] == 2 and с_сост["sources"] == {"И1", "И2"}
        and с_сост["boosters"] == {"Р_СТАРЫЙ", "Р_НОВЫЙ"}, с_сост)
    chk("строки старта не считаются сообщениями канала",
        len(читать_журнал(ж2, {"grpc"})["grpc"]) == 1,
        читать_журнал(ж2, {"grpc"}))
    chk("журнал без строк старта -- пустой состав, а не отказ",
        составы_запусков(ж)["starts"] == 0, составы_запусков(ж))
...
def main() -> int:
    ...
    # Состав ВСЕХ запусков, что есть в журнале: иначе записи прошлого
    # запуска сравниваются с сегодняшним разгонным и объявляются чужими.
    составы = составы_запусков(Path(a.journal))
    if составы["starts"]:
        наши |= составы["sources"]
        разгонные |= составы["boosters"]
        print(f"в журнале записей о запусках: {составы['starts']}, "
               f"их источников {len(составы['sources'])}, "
               f"их разгонных {len(составы['boosters'])}")
