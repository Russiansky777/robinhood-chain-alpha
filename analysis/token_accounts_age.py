...
def main() -> int:
    ...
    def деньги(з):
        # Заперто на счёте: рента плюс остаток, если это WSOL (завёрнутый SOL --
        # это SOL и есть). Остаток прочих минтов деньгами тут не считаем: его
        # цена неизвестна, и выдумывать её нельзя.
        рента = float(з.get("rent_sol") or 0.0)
        остаток = float(з.get("amount_ui") or 0.0) if з.get("mint") == WSOL else 0.0
        return рента + остаток
