...
# подписей: пять наших же покупок за сутки не проверялись вовсе и объявлялись
# чужими, а деплой останавливался словами "мы в кошельке не одни". Цена
# разбора -- один getTransaction на сделку, то есть десятки вызовов раз в
# деплой; цена ложной тревоги -- остановленный деплой и неверное утверждение
# о реальности.
FOREIGN_EXPLAIN_LIMIT = ST.env_int("BLOOM_FOREIGN_EXPLAIN_LIMIT", 40)
...
def подписывали_мы(tx: dict, wallet: str) -> bool:
    """Стоит ли НАША подпись под транзакцией.

    Плательщик и подписант -- не одно и то же: комиссию может платить
    спонсор. Для вопроса "распоряжается ли ключом кто-то ещё" важна именно
    подпись, поэтому смотрим флаг signer у ключей счетов, а при его
    отсутствии -- принадлежность подписи первому счёту (плательщику).
    """
    import bloom_detector as BD  # noqa: PLC0415
    msg = ((tx or {}).get("transaction") or {}).get("message") or {}
    ключи = msg.get("accountKeys") or []
    явные = [k for k in ключи if isinstance(k, dict) and "signer" in k]
    if явные:
        return any(k.get("signer") and k.get("pubkey") == wallet for k in явные)
    плоские = BD._баланс_ключи(tx)
    return bool(плоские) and плоские[0] == wallet


def только_пришло(tx: dict, wallet: str) -> bool:
    """У кошелька в этой транзакции только прибыло: ничего не убыло.

    Это вторая половина признака входящего перевода. Без неё под "входящий"
    попала бы любая чужая транзакция, которая трогает наши счета, -- в том
    числе списание по делегату или чужая упавшая попытка с нашим минтом.
    """
    import bloom_detector as BD  # noqa: PLC0415
    б = BD.балансы_кошелька(tx, wallet)
    if (б.get("native_delta_sol") or 0.0) < 0:
        return False
    прибыло = (б.get("native_delta_sol") or 0.0) > 0
    for з in (б.get("by_mint") or {}).values():
        д = з.get("delta_raw") or 0
        if д < 0:
            return False
        if д > 0:
            прибыло = True
...
def объяснить_свежие(helius, свежие: list, минты: set, wallet: str, *,
    ...
    """
    import bloom_detector as BD  # noqa: PLC0415
    предел = FOREIGN_EXPLAIN_LIMIT if предел is None else предел
        ...
        if упали:
            ...
            объяснённые.append({**зап, "mints": наши_минты_в_tx,
                                 "why": "неудачная попытка выхода по нашей позиции: "
                                        ...
                                        "в счетах наш минт"})
        elif (not ошибка and not подписывали_мы(tx, wallet)
               and только_пришло(tx, wallet)):
            # Входящий перевод. Чтобы прислать SOL или токен на адрес, подпись
            # владельца адреса НЕ нужна: платит и подписывает отправитель.
            # Значит такая транзакция не доказывает, что ключом распоряжается
            # кто-то ещё, и называть её "мы в кошельке не одни" -- неправда.
            # Проверяется именно подпись, а не плательщик: плательщиком может
            # быть спонсор комиссии при нашей подписи. И требуется, чтобы у нас
            # только ПРИБЫЛО: чужая транзакция, в которой наш остаток упал (или
            # которая упала с ошибкой), входящим переводом не является и
            # остаётся необъяснённой.
            объяснённые.append({**зап, "mints": наши_минты_в_tx,
                                 "why": "входящий перевод: нашей подписи в "
                                        "транзакции нет, распоряжаться кошельком "
                                        "она не требует"})
        else:
            # Осталось то, что действительно подписано нашим ключом и в журнале
            # не значится. Здесь ничего не списывается на "наверное площадка":
            # в запись кладутся факты (плательщик, программы, сдвиг SOL), чтобы
            # владелец видел, ЧТО именно произошло его ключом.
            meta = tx.get("meta") or {}
            сдвиг = None
            try:
                и = ключи.index(wallet)
                сдвиг = round((int((meta.get("postBalances") or [])[и])
                                - int((meta.get("preBalances") or [])[и])) / 1e9, 9)
            except (ValueError, IndexError, TypeError):
                сдвиг = None
            необъяснённые.append({**зап, "fee_payer": (ключи[0] if ключи else None),
                                   "signed_by_us": True,
                                   "native_delta_sol": сдвиг,
                                   "programs": BD.программы_в_транзакции(tx)[:8],
                                   "why_not": "подписано нашим ключом, в журнале "
                                              "решений и позиций не значится"})
    # НЕ ПРОВЕРЕННЫЕ из-за предела -- это не "чужие", а "неизвестные". Слова
    # должны совпадать с делом: сказать "мы в кошельке не одни" про сделку, на
    # которую мы даже не посмотрели, значит соврать о реальности. Старт они
    # по-прежнему блокируют (неясность -- запрет), но своей строкой.
    не_проверены = свежие[предел:] if len(свежие) > предел else []
    return {"explained": объяснённые, "unexplained": необъяснённые,
             "not_checked": не_проверены, "failures": сбои,
             "checked": min(len(свежие), предел), "limit": предел}
...
def reconcile(state: ST.ExecState, *, mode: str, helius=None,
    ...
    if helius is not None:
        ...
        if чужая.get("recent"):
            ...
            чужая["not_checked"] = разбор.get("not_checked") or []
            чужая["not_checked_count"] = len(чужая["not_checked"])
            чужая["explain_limit"] = разбор.get("limit")
    ...
    if чужая.get("known") is False:
        ...
        if чужая.get("explained"):
            # Заметка ГРУППИРУЕТСЯ по причине. Одна строка на всех объясняла
            # входящий перевод владельца словами "выход по нашей позиции" --
            # объяснение было верным в записи и неверным в докладе, а читают
            # доклад. Слова должны совпадать с делом в обоих местах.
            по_причине: dict = {}
            for x in чужая["explained"]:
                по_причине.setdefault(x.get("why") or "без причины", []).append(x)
            for причина, строки in sorted(по_причине.items(),
                                           key=lambda kv: -len(kv[1])):
                заметки.append(
                    f"{len(строки)} свежих сделок: {причина} -- "
                    + ", ".join(
                        f"{x['signature'][:12]}"
                        + (f" ({','.join(x['mints'])})" if x.get("mints") else "")
                        for x in строки))
        if чужая.get("not_checked_count"):
            # Своя строка и свои слова: это неизвестность из-за предела, а не
            # доказанная чужая активность.
            блокеры.append(
                f"{чужая['not_checked_count']} свежих сделок НЕ ПРОВЕРЕНЫ: предел "
                f"разбора {чужая.get('explain_limit')}. Это неизвестность, а не "
                "доказанная чужая активность -- старт блокируется, потому что "
                "неясность трактуется как запрет")
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # Предел разбора: сделки СВЕРХ предела -- "не проверены", а не "чужие".
        # Пока это не различалось, деплой останавливался словами "мы в кошельке
        # не одни" про наши же покупки, на которые никто не смотрел.
        много = [{"signature": f"S{i}", "ts": 1000.0 + i} for i in range(5)]
        разбор_п = объяснить_свежие(HeliusСПродажей([]), много, {"МИНТ"},
                                     ST.EXECUTOR_WALLET, предел=2)
        chk("сверх предела сделки помечены как НЕ проверенные",
            len(разбор_п.get("not_checked") or []) == 3
            and разбор_п["checked"] == 2, разбор_п)
        chk("и в необъяснённые они НЕ попали",
            len(разбор_п["unexplained"]) == 2, разбор_п["unexplained"])
        chk("и объяснение названо в примечаниях своей причиной",
            any("авто-ордера Bloom" in n and "выход по нашей позиции" in n
        ...
        chk("сделка по чужому минту объяснением не считается",
            ...
            r2["foreign_activity"])
        chk("необъяснённая запись несёт факты: плательщик, программы, сдвиг SOL",
            (r2["foreign_activity"]["unexplained"][0].get("signed_by_us") is True
             and "programs" in r2["foreign_activity"]["unexplained"][0]),
            r2["foreign_activity"]["unexplained"][0])

        # ВХОДЯЩИЙ ПЕРЕВОД. Чтобы прислать SOL на адрес, подпись владельца
        # адреса не нужна. Такая транзакция не доказывает, что ключом
        # распоряжается кто-то ещё, и блокировать старт словами "мы в кошельке
        # не одни" она не должна. Ровно так выглядело пополнение 24.09 11:20Z.
        class HeliusВходящий(HeliusЗаглушка):
            def транзакция(self, подпись, **kw):
                чужой = "DNJeTYni5QQVCwNNt1SrHu1GjYR4ak5Zw7LeuFsKYmXX"
                return {"slot": 9,
                        "transaction": {"message": {
                            "accountKeys": [{"pubkey": чужой, "signer": True},
                                             {"pubkey": ST.EXECUTOR_WALLET,
                                              "signer": False}],
                            "instructions": [
                                {"programId": "11111111111111111111111111111111"}]}},
                        "meta": {"err": None, "fee": 5000,
                                  "preBalances": [10 ** 9, 10 ** 8],
                                  "postBalances": [10 ** 9 - 2 * 10 ** 9, 2 * 10 ** 9],
                                  "preTokenBalances": [], "postTokenBalances": [],
                                  "innerInstructions": []}}

        разбор_вх = объяснить_свежие(
            HeliusВходящий([]), [{"signature": "ПОПОЛНЕНИЕ", "ts": 1000.0}],
            {"МИНТ"}, ST.EXECUTOR_WALLET)
        # Заметка в докладе группируется по причине: входящий перевод не
        # должен попадать в строку про выходы по нашим позициям.
        r_см = reconcile(st, mode=ST.MODE_LIVE_TEST, helius=HeliusВходящий(
            [{"signature": "ПОПОЛНЕНИЕ", "blockTime": int(сейчас - 60)}],
            баланс=0.35), allow_open_live_test=True)
        chk("входящий перевод в докладе назван входящим, а не выходом",
            any("входящий перевод" in n for n in r_см["notes"])
            and not any("входящий перевод" in n and "выход по нашей позиции" in n
                        for n in r_см["notes"]), r_см["notes"])

        chk("входящий перевод объяснён: нашей подписи в нём нет",
            len(разбор_вх["unexplained"]) == 0
            and "входящий перевод" in разбор_вх["explained"][0]["why"], разбор_вх)
        # Чужая транзакция, в которой у нас УБЫЛО, входящим переводом не
        # считается: иначе списание по делегату прошло бы как пополнение.
        class HeliusСписание(HeliusЗаглушка):
            def транзакция(self, подпись, **kw):
                чужой = "ЧУЖОЙ_ПОДПИСАНТ"
                бал = lambda raw: {"accountIndex": 1, "mint": "МИНТ",
                                    "owner": ST.EXECUTOR_WALLET,
                                    "uiTokenAmount": {"amount": str(raw),
                                                       "decimals": 6,
                                                       "uiAmount": raw / 1e6}}
                return {"slot": 9,
                        "transaction": {"message": {
                        ...
                        "meta": {"err": None, "fee": 5000,
                                  "preBalances": [10 ** 9, 10 ** 8],
                                  "postBalances": [10 ** 9, 10 ** 8],
                                  "preTokenBalances": [бал(92_000000)],
                                  "postTokenBalances": [бал(0)],
                                  "innerInstructions": []}}

        разбор_сп = объяснить_свежие(
            HeliusСписание([]), [{"signature": "СПИСАНИЕ", "ts": 1000.0}],
            {"НЕ_НАШ"}, ST.EXECUTOR_WALLET)
        chk("чужая транзакция, в которой у нас УБЫЛО, входящим не считается",
            len(разбор_сп["unexplained"]) == 1, разбор_сп)

        chk("подпись проверяется по флагу signer, а не по плательщику",
            подписывали_мы(HeliusВходящий([]).транзакция("X"),
                            ST.EXECUTOR_WALLET) is False)
