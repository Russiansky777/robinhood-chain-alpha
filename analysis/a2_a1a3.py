...
def main() -> None:
    ...
    for v in входы.values():
        v["порча_множителя"] = (1.0 + v["наценка"]) / (1.0 - скидка)
    ...
    out = {
        ...
        "издержки_модели": изд.как_словарь() | {
            "скидка_выхода": скидка, "налоговых_переводов": переводов,
            "порча_множителя": "своя у каждого входа, см. раздел входы"},
        "входы": входы,
