...
class Seller:
    ...
    def обработать_полосу(self, pos: dict, *, now: float,
        ...
        2) продаётся РОВНО количество, купленное полосой (lane_bought_raw), и
           никогда весь остаток минта. С 25.09 у полосы свой кошелёк, и покупки
           Bloom в нём нет -- но остаток всё равно может быть больше купленного
           (недопроданный хвост прошлой попытки на том же минте), и продавать
           его этой позицией значило бы посчитать чужую выручку своей;
        ...
        if not isinstance(куплено, int) or куплено <= 0:
            ...
            if ждём is not None and ждём < self.ждать_количество_s:
                ...
                return итог
            # ИТОГ НЕСЧИТАЕМ (уточнение владельца 25.09). Количество не
            # добралось -- значит и итог этой пары посчитать нечем. Это не
            # утечка денег и не повод останавливать полосу: помечаем пару и
            # идём дальше. Замер скорости у неё остаётся годным.
            ...
            return self._полоса_unsold(
                ...
                причина=("количество, купленное полосой, неизвестно -- весь "
                          "остаток минта продавать нельзя: не наша выручка"))
