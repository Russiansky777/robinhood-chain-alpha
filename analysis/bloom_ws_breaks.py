...
НАША_СТОРОНА = ("таймаут пинга", "переподписка", "сеть:")
...
def причина_из_текста(тип: str, текст: str) -> str:
...
def разобрать_журнал(строки: list) -> dict:
    ...
    for строка in строки:
        ...
        if м:
            ...
            причина = причина_из_текста(тип, текст)
...
def в_таблицу(из_лога: dict, из_решений: dict | None = None) -> str:
    ...
    for причина, з in (о.get("by_reason") or {}).items():
        ...
        строки.append(f"| {причина} | {з['n']} | {з['blame']} | {пример[:120]} |")
