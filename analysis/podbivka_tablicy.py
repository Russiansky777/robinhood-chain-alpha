...
def таблица_порогов(заголовок: str, строки: list) -> list:
    ...
    out = [f"### {заголовок}", "",
           ...
           "| источник | порог | n (сим) | S+0/72 | S+1/72 | S+2/72 | S+0/150 | S+1/150 | S+2/150 | без свопов 72 / 150 | продал в окне | толпа мед | метка |",
           "|---|---|---|---|---|---|---|---|---|---|---|---|---|"]
    for имя, порог, к in строки:
        def д(x):
            return "—" if x is None else f"{x:.0%}"
        out.append(f"| {имя} | {порог} | {к['покупок']} ({к['сим_ok']}) | {ячейка(к['S0_72'])} | {ячейка(к['S1_72'])} | "
