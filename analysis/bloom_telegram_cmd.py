...
# Сколько чужих чатов помним. Больше не нужно: ищем ровно один журнальный,
# а неограниченный список раздувал бы признак жизни.
ПРЕДЕЛ_ВИДЕННЫХ_ЧАТОВ = 10
...
class Команды:
    ...
    def __init__(self, состояние: ST.ExecState, *, статус_фн=None,
        ...
        self.принято = 0
        self.чужих = 0
        # Чужие чаты, из которых приходили сообщения: ид -> сведения.
        self.виденные_чаты: dict = {}
        self.ошибок = 0
    ...
    def заметить_чат(self, сообщение: dict) -> None:
        """Записать, из какого ЧУЖОГО чата пришло сообщение. Без действий.

        Нужно ровно для одного дела: найти ID журнальной группы, которую
        владелец создал, НЕ запуская второй getUpdates рядом с этим. Два
        опроса одного бота отбирают обновления друг у друга, и второй
        клиент мог бы съесть команду владельца.

        Команды отсюда не исполняются никогда: проверка владельца стоит
        отдельно и ниже. Это только наблюдение, и оно ограничено по числу
        записей -- чужой чат не должен раздувать признак жизни.
        """
        чат = сообщение.get("chat") or {}
        ид = str(чат.get("id") or "")
        if not ид or ид == владелец():
            return
        if ид not in self.виденные_чаты and len(self.виденные_чаты) >= ПРЕДЕЛ_ВИДЕННЫХ_ЧАТОВ:
            return
        строка = self.виденные_чаты.setdefault(ид, {"count": 0})
        строка["count"] += 1
        строка["type"] = чат.get("type")
        строка["title"] = str(чат.get("title") or чат.get("username") or "")[:60]
        строка["last_utc"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    ...
    def обработать_обновления(self, обновления: list, *, отправить=None) -> list:
        ...
        for u in обновления or []:
            ...
            # Чат замечается ДО проверки на команду: в журнальной группе
            # владелец пишет обычное сообщение, а не /команду, и по "/" оно
            # бы отсеялось вместе с единственным способом узнать её ID.
            self.заметить_чат(сообщение)
    ...
    def признак_жизни(self) -> dict:
        ...
        return {"enabled": ок, "why_not": почему, "accepted": self.принято,
                ...
                "offset": self.offset(),
                # Чужие чаты, из которых приходили сообщения. Ровно для
                # того, чтобы найти ID журнальной группы и положить его в
                # TELEGRAM_LOG_CHAT_ID. Команды из них не исполняются.
                "seen_chats": {к: dict(v) for к, v in self.виденные_чаты.items()}}
...
def self_test() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory() as d:
            ...
            # --- ЖУРНАЛЬНАЯ ГРУППА: ID запоминается, команды -- нет
            os.environ["TELEGRAM_CHAT_ID"] = "555"
            к6 = Команды(st)
            к6.запомнить_offset(1)
            ответы6 = к6.обработать_обновления([
                {"update_id": 10, "message": {
                    "chat": {"id": "-1001234567890", "type": "supergroup",
                              "title": "Журнал детектора"},
                    "from": {"id": "555"}, "text": "проверка"}},
                {"update_id": 11, "message": {
                    "chat": {"id": "-1001234567890", "type": "supergroup",
                              "title": "Журнал детектора"},
                    "from": {"id": "555"}, "text": "/kill"}}])
            chk("ID журнальной группы запомнен по обычному сообщению",
                "-1001234567890" in к6.виденные_чаты
                and к6.виденные_чаты["-1001234567890"]["title"] == "Журнал детектора"
                and к6.виденные_чаты["-1001234567890"]["count"] == 2,
                к6.виденные_чаты)
            chk("команда ИЗ ЖУРНАЛЬНОЙ группы не исполняется",
                ответы6 == [] and к6.принято == 0 and к6.чужих == 1,
                (ответы6, к6.принято, к6.чужих))
            kill_после = st.kill_active()[0]
            chk("и рубильник от неё не сработал", kill_после is False, kill_после)
            к6.обработать_обновления([
                {"update_id": 12, "message": {
                    "chat": {"id": "555", "type": "private"},
                    "from": {"id": "555"}, "text": "/status"}}])
            chk("свой чат в список чужих не попадает",
                "555" not in к6.виденные_чаты, к6.виденные_чаты)
            for н in range(ПРЕДЕЛ_ВИДЕННЫХ_ЧАТОВ + 5):
                к6.заметить_чат({"chat": {"id": f"чужой{н}", "type": "group"}})
            chk("список чужих чатов ограничен",
                len(к6.виденные_чаты) <= ПРЕДЕЛ_ВИДЕННЫХ_ЧАТОВ,
                len(к6.виденные_чаты))
            chk("чужие чаты видны в признаке жизни",
                "seen_chats" in к6.признак_жизни())
    finally:
        os.environ.clear()
