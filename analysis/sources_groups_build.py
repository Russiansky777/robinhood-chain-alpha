...
# СКОЛЬКО ВСЕГО АДРЕСОВ В ГРУППЕ СКОРОСТИ. Решение владельца 25.09 по кредитам:
# "Если прогноз > 300 000 -- сразу сократить speed_only до 8 кошельков с
# наибольшим числом попаданий в S+0 (не наших), остальное оставить". Замер,
# который это включил: с 80 источниками детектор сжёг 44 445 кредитов за 52
# минуты (1281 МБ трафика подписки), то есть около 51 300 в час и ~1.2 млн в
# сутки против потолка 300 000.
#
    ...
    return {
        ...
        "groups": {
            ...
            "speed_only": {
                ...
                "credits_rule": ("сокращено до 8 адресов с наибольшим числом "
                                  "попаданий в S+0 по слову владельца: прогноз "
                                  "расхода детектора с 80 источниками был ~1.2 "
                                  "млн кредитов в сутки против потолка 300 000"),
