    ...
    return {
        ...
        "groups": {
            "bloom_lane": {
                "lane_sol": 0.05,
                "bloom_trades": True,
                # РАЗМЕР BLOOM ПО BATCH-3/5 -- 0.05, А НЕ 0.2 (решение владельца
                # 26.09, пункт 9): "Bloom на bloom_lane 0.2 -> 0.05 при снятии
                # KILL. Пары сохраняются, ночной расход площадки режется
                # вчетверо". До этого поля здесь не было вовсе, и Bloom брал
                # общий BLOOM_BUY_SOL из окружения (0.2).
                "bloom_sol": 0.05,
                "fanout": True,
                "note": ("BATCH-3/5 плюс лидер: полоса 0.05 SOL (пункты 2 и 3). "
                          "Bloom по этой группе тоже 0.05 SOL -- решение владельца "
                          "26.09 (пункт 9): было 0.2, пары Bloom против полосы "
                          "сохраняются, а ночной расход площадки режется вчетверо. "
                          "Адресов здесь нет нарочно -- "
            ...
            # ГРУППА КАНДИДАТОВ (решение владельца 26.09). Адреса приходят от
            # второй сессии по подбивке чужих кошельков: десять лучших по чистому
            # S+1/72 при не менее пяти покупках и доле "источник продал в окне"
            # ниже 30 %. Пока список пуст, группа ничего не меняет.
            "candidates": {
                "lane_sol": 0.01,
                "bloom_trades": False,
                "bloom_sol": None,
                ...
                "stop_loss_sol": 0.2,
                "min_target_sol": 2.0,
                "note": ("Кандидаты из подбивки чужих кошельков: полоса 0.01 SOL, "
                          "Bloom по ним НЕ торгует, порог покупки источника 2 SOL, "
                          "потолок оборота 1 SOL в сутки, стоп -0.2 SOL."),
                "addresses": {},
            },
            "speed_only": {
                "lane_sol": 0.01,
                # BLOOM ТОРГУЕТ И ПО ГРУППЕ СКОРОСТИ (решение владельца 25.09,
                # ночь): "Bloom включить на 30 speed_only с размером 0.01 SOL
                # (порог источника 0.5 как у полосы). Цель -- пары на каждой
                # покупке скорости". Причина прямая: 38 кошельков lane_only
                # дают примерно одну покупку в два часа, и пар за ночь было бы
                # единицы вместо двадцати, а частота вся здесь.
                "bloom_trades": True,
                "bloom_sol": 0.01,
                "fanout": False,
                # ПОТОЛОК И СТОП ГРУППЫ СКОРОСТИ. 25.09 вечером владелец дал
                # "потолок 1 SOL в сутки (покупки + чаевые), стоп -0.5 SOL"
                # (было 0.2 и -0.15). 26.09 потолок поднят до 3 SOL: оборот
                # группы упёрся в 1.020627 к 14:03Z, и полоса по скорости
                # встала сама -- а на ней идут живые проверки.
                "day_cap_sol": 3.0,
                "stop_loss_sol": 0.5,
