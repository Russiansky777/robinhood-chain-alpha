...
def вердикт_dbot(записи: list, *, источник: str, минт: str,
    ...
    if not кандидаты:
        return {"итог": "не_видел",
                 "пояснение": (f"записи DBot по этому источнику и минту в пределах "
                               f"{окно_с:.0f} с не найдено")}
    кандидаты.sort(key=lambda x: x[0])
    d, r = кандидаты[0]
    причина = r.get("skipReason")
    return {"итог": "купил" if not причина else "отказал",
             "код_dbot": причина or "ПРОШЛО",
             "состояние": r.get("state"),
             "ошибка": r.get("errorMessage") or None,
...
def записи_dbot(ключ: str, задачи: tuple, конфиг: Path) -> list:
    ...
    res = ((конф or {}).get("тело") or конф or {}).get("res") or []
    out = []
    for t in res:
...
def разбор(tx: dict, источник: str) -> dict:
    ...
    return {
        ...
        "нынешний_разбор_натив_sol": б["нативный_delta_sol"],
        "нынешний_тип": сиг.get("тип"),
        "нынешняя_причина": сиг.get("решение_причина"),
        "минт": сиг.get("минт"),
        "токенные_дельты": {m: з.get("delta_ui") for m, з in б["по_минтам"].items()},
    }
...
def main() -> int:
    ...
    if a.self_test:
        ...
        chk("и потому зовёт сделку неоднозначной",
            р["нынешний_тип"] == "ambiguous", р["нынешний_тип"])
        ...
        chk("вердикт DBot найден по источнику и минту", в["итог"] == "купил", в)
        ...
        chk("по другому минту -- не видел", в3["итог"] == "не_видел", в3)
        в4 = вердикт_dbot(зап, источник="ИСТ", минт="М", время_сделки=99_000.0)
        chk("вне окна -- не видел", в4["итог"] == "не_видел", в4)
    ...
    итог = {"источник": a.source, "записей_dbot": len(записи),
    ...
    for s in a.sig:
        ...
        if tx is None:
            итог["сделки"].append({"подпись": s, "почему": "getTransaction не отдал"})
            continue
        р = разбор(tx, a.source)
        if записи and р.get("минт"):
            р["dbot"] = вердикт_dbot(записи, источник=a.source, минт=р["минт"],
                                      время_сделки=(tx.get("blockTime") or 0))
        итог["сделки"].append({"подпись": s, **р})
    живые = [c for c in итог["сделки"]
              if (c.get("dbot") or {}).get("итог") == "купил"]
