...
def прогнать(путь: Path = СНИМОК, *, порог: float = 2.0) -> dict:
    ...
    лучший = max(оба, key=lambda k: (оба[k]["доля_совпадений"] or 0))
    return {
        ...
        "порядок_фильтров_лучше_совпал": лучший,
        "прогоны": оба,
        "оговорки": [
...
def main() -> int:
    ...
    лучший = отчёт["порядок_фильтров_лучше_совпал"]
    пр = отчёт["прогоны"][лучший]
