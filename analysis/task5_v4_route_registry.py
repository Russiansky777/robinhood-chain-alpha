...
ЧЕСТНО О СЛОВЕ "ФИД" (внешнее ревью, второй раунд, 2026-09-13): в
формулировках владельца ниже (цитаты) встречается "из фида в реальном
времени" -- это ЕГО постановка задачи, не описание фактической
реализации. РЕАЛЬНЫЙ источник сигналов здесь и в task5_v4_hotpath.py --
периодический RPC-опрос `eth_getLogs` (через alchemy_fallback.
_chunked_get_logs, каждые POLL_INTERVAL_S/DISCOVERY_POLL_INTERVAL_S),
НЕ WebSocket-подписка (`eth_subscribe`) и НЕ клиент sequencer-фида
(task5_bot_feed_client.py::SequencerFeedClient, который в этом модуле
НЕ импортируется и не используется). Называть это "событиями из
sequencer feed" было бы неточно -- не делаем этого нигде в коде/логах."""
