...
ПОТОЛКИ = (0, 1)
ОБРАЗЦОВ = 5
# Ниже этого числа сообщений вывод не делается. Первый прогон поймал три
# сообщения за десять минут -- и на них замер бодро заявил "разницы нет".
# Три сообщения не доказывают ни наличия разницы, ни её отсутствия, и
# такой вердикт хуже отсутствия вердикта: он закрывает вопрос.
МИНИМУМ_СООБЩЕНИЙ = 25
...
def сравнение(счёт: Счёт) -> dict:
    ...
    мало = (ca["messages"] + cb["messages"]) < МИНИМУМ_СООБЩЕНИЙ
    итог["sample_too_small"] = мало
    if not общие:
        итог["verdict"] = ("общих подписей нет -- за окно ни один источник не "
                           "сделал сделки, вывод делать не на чем")
    elif мало and not починено:
        итог["verdict"] = (f"выборка мала: сообщений {ca['messages']}+"
                           f"{cb['messages']} при минимуме {МИНИМУМ_СООБЩЕНИЙ}, "
                           "и ни одного сообщения без meta не поймано. Это НЕ "
                           "означает, что потолок ни при чём -- окно просто не "
                           "застало нужный случай. Нужен прогон длиннее")
...
def self_test() -> int:
    ...
    # маленькая выборка не даёт вердикта "разницы нет"
    с2 = Счёт()
    с2.учесть(0, res(sig=подпись("C"), meta=True))
    с2.учесть(1, res(sig=подпись("C"), meta=True))
    ср2 = сравнение(с2)
    chk("на двух сообщениях вердикта о причине нет",
        ср2["sample_too_small"] is True and "выборка мала" in ср2["verdict"],
        ср2["verdict"])

    # при достаточной выборке и отсутствии разницы -- вывод делается
    с2б = Счёт()
    for i in range(15):
        с2б.учесть(0, res(sig=подпись("C"), meta=False))
        с2б.учесть(1, res(sig=подпись("C"), meta=False))
    ср2б = сравнение(с2б)
    chk("при достаточной выборке без разницы отправляет искать другую причину",
        ср2б["sample_too_small"] is False and "другую причину" in ср2б["verdict"],
        ср2б["verdict"])
