...
def остановить_полосу(состояние, причина: str) -> dict:
    """Рубильник ТОЛЬКО полосы при денежной странности (владелец 25.09).

    Одна точка на модуль: иначе часть странностей останавливала бы полосу, а
    часть -- нет. Bloom и тень не трогаются вовсе.
    """
...
ПОТОЛОК_РАСХОДА_КОНТРОЛЯ_SOL = 0.03
...
def self_test() -> int:
    ...
    with _tmp.TemporaryDirectory() as вр_к:
        ...
        while прошло_ < ЛИМИТ_КОНТРОЛЕЙ_В_СУТКИ and можно_контроль(
            ...
            прошло_ += 1
        # 0.03 SOL при цене контроля 0.002005 SOL -- это ровно 14 штук:
        # пятнадцатый вышел бы за потолок и потому не пускается.
        chk("потолок 0.03 SOL пускает 14 контролей и закрывает пятнадцатый",
            прошло_ == 14 and можно_контроль(ск2, цена_лампорты=цк)["ok"] is False,
            (прошло_, расход_контроля(ск2)))
        chk("потолок сработал РАНЬШЕ суточного числа -- он и есть тесный",
            прошло_ < ЛИМИТ_КОНТРОЛЕЙ_В_СУТКИ
            and "потолок" in (можно_контроль(ск2, цена_лампорты=цк)["why_not"] or ""),
            можно_контроль(ск2, цена_лампорты=цк))
