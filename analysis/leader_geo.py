...
Длина слота взята ИЗМЕРЕННОЙ, а не номинальной: 267.7 мс -- медиана по 63
парам "слот -- UTC" из наших же покупок. Номинальные 400 мс завышали оценку
в полтора раза, и до 26.09 03:2xZ стояли именно они.
"""
...
# ДЛИНА СЛОТА -- ИЗМЕРЕНА, А НЕ ВЗЯТА ИЗ УЧЕБНИКА. По 63 парам наших же
# покупок (слот и UTC в одной строке журнала, data/lane_table.json) выходит
# медиана 0.2677 с на слот, разброс 0.260-0.276. Номинальные 400 мс завышали
# любую оценку задержки в полтора раза, и именно они стояли здесь до 26.09
# 03:2xZ. Проверка: 87 269 слотов между 25.09 18:09:41Z и 26.09 00:39:15Z
# заняли 23 374 с, то есть 0.2678 с на слот.
СЛОТ_МС = 267.7
...
def добор_из_таблицы(ряды: list, путь: Path) -> dict:
    """Наш слот и место в блоке для покупок ПОЛОСЫ -- из таблицы полосы.

    ПОЧЕМУ ЭТО НУЖНО. В журнале позиций у полосы нашего слота нет вовсе: там
    лежит только source_slot, а слот и место своей транзакции полоса не пишет.
    Первый прогон II.14 из-за этого показал по полосе S+0 = 0 и "в цели
    считаемо 0" -- это был пробел данных, а не свойство географии. Слот и место
    по цепи уже добирает lane_table.py (getTransaction по подписям вариантов),
    и его выгрузка data/lane_table.json хранит их по тому же
    client_order_id. Берём их оттуда -- одно измерение, а не второе.

    Возвращает счёт: сколько строк добрано, сколько осталось без слота.
    """
    счёт = {"добрано": 0, "без_слота": 0, "нет_в_таблице": 0}
    таблица: dict = {}
    if путь and путь.exists():
        д = json.loads(путь.read_text(encoding="utf-8"))
        for с in (д.get("rows") or []):
            if с.get("cid"):
                таблица[с["cid"]] = с
    for з in ряды:
        if з["чья"] != "полоса" or з.get("our_slot"):
            continue
        с = таблица.get(з.get("cid"))
        if not с:
            счёт["нет_в_таблице"] += 1
            счёт["без_слота"] += 1
            continue
        цепь = с.get("chain") or {}
        слот = с.get("our_slot") or цепь.get("slot")
        if not слот:
            счёт["без_слота"] += 1
            continue
        з["our_slot"] = слот
        if з.get("block_index") is None:
            з["block_index"] = с.get("block_index")
        if з.get("block_total") is None:
            з["block_total"] = с.get("block_total")
        з["слот_откуда"] = "цепь (таблица полосы)"
        счёт["добрано"] += 1
    return счёт
...
def main() -> int:
    ...
    р.add_argument("--lane-table", default="data/lane_table.json",
                   help="выгрузка lane_table.py: оттуда берётся слот и место полосы по цепи")
    ...
    доб = добор_из_таблицы(ряды, Path(а.lane_table) if а.lane_table else None)
    print(f"слот полосы добран из таблицы: {доб['добрано']}; "
          f"осталось без слота: {доб['без_слота']} "
          f"(из них нет в таблице: {доб['нет_в_таблице']})")
    ...
    куски = [f"# II.14 География лидера слота, покупки с {а.since}", "",
              ...
              f"У полосы своего слота в журнале нет: он добран по цепи из таблицы "
              f"полосы для {доб['добрано']} покупок, без слота осталось "
              f"{доб['без_слота']} (их нет ни в одной строке, а не посчитаны нулём).", "",
              f"«Сел → увидели» -- оценка: отставание слотов x {СЛОТ_МС:.1f} мс "
              "(длина слота измерена по нашим покупкам, не номинальная) + возраст "
