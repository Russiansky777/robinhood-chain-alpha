    ...
    def сторона_b():
        try:
            if первый == "B":
                ...
                событие.wait(5.0)
            t = time.time()
            # без_учёта=True: у синтетического теста СВОЙ учёт и свои
            # пределы (пар всего, предел трат, сверка денег по цепи). Он не
            # полоса детектора и её суточные пределы занимать не должен.
            о = OS.отправить(своя_b64, ключ_операции=ключ_операции,
                              отправитель=отправитель, без_учёта=True)
...
def self_test() -> int:
    ...
    try:
        ...
        повтор = OS.отправить("AAA", ключ_операции="пара-1",
                               отправитель=sender_заглушка, без_учёта=True)
    ...
    chk("без BLOOM_OWN_SEND_LIVE сторона B в сеть не идёт",
        OS.отправить("AAA", ключ_операции="пара-хх",
                      отправитель=sender_заглушка,
                      без_учёта=True).get("sent") is False
