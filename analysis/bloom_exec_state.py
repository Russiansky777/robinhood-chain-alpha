...
# ОТКАЗ, КОТОРЫЙ КАСАЕТСЯ ТОЛЬКО ПЛОЩАДКИ. Своим кодом, а не общим: по общему
# детектор глушил и полосу -- решение помечалось skip, а полоса запускается
# только на buy. Замер 26.09 (окно KILL_BLOOM 14:05:50-14:16:00Z): отказов
# площадки 7, покупок полосы 0.
КОД_РУБИЛЬНИК_BLOOM = "KILL_SWITCH_BLOOM"
...
def расход_отправки(поз: dict) -> float:
    """Сколько эта покупка стоила кошельку СВЕРХ купленной суммы.
    ...
    ЧАЕВЫЕ УХОДЯТ ТОЛЬКО С УДАВШЕЙСЯ ТРАНЗАКЦИИ. Перевод чаевых -- инструкция
    внутри той же транзакции: упала транзакция -- перевода не было, платится
    только приоритет и базовый тариф. Живой пример 26.09 00:04:23Z: покупка
    0.05 отклонена пулом (Custom 6040), с кошелька полосы списано ровно
    0.001005 SOL, а чаевые 0.001 остались на месте.
    """
    п = поз or {}
    из_ = 0.0
    чаевые = п.get("lane_tips_total_sol")
    if чаевые and п.get("chain_ok") is not False:
        из_ += float(чаевые)
    приоритет = п.get("lane_priority_lamports")
    if приоритет:
        из_ += float(приоритет) / 1_000_000_000.0
    ...
    if п.get("lane_signature") or п.get("ts_sent") or п.get("signatures"):
        из_ += 5000 / 1_000_000_000.0
    return из_


def делит_минт(cid: str, поз: dict, все: dict) -> str:
    """Делила ли эта пара свой минт с ДРУГОЙ позицией той же полосы.

    Продажа несчитаемой пары идёт по остатку кошелька. Если в то же окно на
    кошельке висела ещё одна позиция того же минта, возврат принадлежит им
    обеим, и разложить его нечем. Возвращает client_order_id такой позиции
    (строкой, для записи в журнал) или пустую строку.

    Окна сравниваются по ts_intent (открытие) и ts_closed (закрытие); у ещё не
    закрытой позиции конец окна -- бесконечность, а не "сейчас": она всё ещё
    держит тот же минт.
    """
    минт = (поз or {}).get("mint")
    if not минт:
        return ""
    начало = float((поз or {}).get("ts_intent") or 0.0)
    конец = float((поз or {}).get("ts_closed") or 0.0) or float("inf")
    for чужой, о in (все or {}).items():
        # Чужая полоса не в счёт: у Bloom свой кошелёк, и его остаток к
        # остатку полосы отношения не имеет.
        if чужой == cid or (о or {}).get("lane") != (поз or {}).get("lane"):
            continue
        if (о or {}).get("mint") != минт:
            continue
        о_начало = float(о.get("ts_intent") or 0.0)
        о_конец = float(о.get("ts_closed") or 0.0) or float("inf")
        if о_начало <= конец and о_конец >= начало:
            return str(чужой)
    return ""


def итог_позиции(поз: dict) -> tuple:
    """Итог закрытой позиции и её расход на отправку: (итог, расход).

    ОДНА ФОРМУЛА НА РЕПОЗИТОРИЙ (I.5). Отсюда её берут и суточный счёт
    состояния, и суточный счёт полосы: когда их было две, числа расходились.

    Итог = возврат - вход - расход на отправку. Расход вычитается ВСЕГДА, даже
    когда покупка не села: с кошелька эти деньги уже ушли.

    Возвращает (None, расход), если вход или возврат неизвестны: выдумывать
    ноль нельзя -- ноль читался бы как "вышли в ноль".
    """
    п = поз or {}
    расход = расход_отправки(п)
    вход = п.get("sol_in")
    возврат = п.get("closed_sol_net")
    if возврат is None:
        возврат = (п.get("last_sell_outcome") or {}).get("sol_delta_net")
    # СЧЁТ ПО ЦЕПИ ИМЕЕТ ПРИОРИТЕТ НАД СЧЁТОМ ПО ПОЛЯМ. Поля не видят платы за
    # создание счетов: живая покупка 25.09 18:09:41Z стоила кошельку
    # 0.053493440, а поля дают 0.052005 -- разница 0.001488440. На 63 сделках
    # ночи это 0.09 SOL расхода мимо суточного стопа. Когда в позиции есть
    # нативная дельта самой покупки, итог считается по ней:
    #     итог = возврат + натив - комиссия покупки
    # (натив отрицателен и уже включает вход, чаевые и плату за счета).
    натив = п.get("lane_buy_native_sol")
    комиссия_покупки = п.get("lane_buy_fee_sol")
    if натив is not None:
        расход = round(-float(натив) - float(вход or 0.0)
                       + float(комиссия_покупки or 0.0), 9)
        if возврат is None:
            return None, расход
        return (float(возврат) + float(натив) - float(комиссия_покупки or 0.0),
                расход)
    # ПОКУПКА ОТКЛОНЕНА ЦЕПЬЮ -- ВХОД НЕ УХОДИЛ. У отклонённой транзакции
    # откатываются все переводы: ни входа, ни чаевых с кошелька не списано,
    # платится только приоритет и тариф. Вычитать вход тут значило бы
    # выдумать убыток размером со сделку (26.09 00:04:23Z -- -0.051005
    # вместо -0.001005).
    if п.get("chain_ok") is False:
        return -расход, расход
    if п.get("result_uncountable"):
        # I.4 (поручение владельца 25.09: "итог полосы без количества -- по
        # возврату и входу; несчитаемых не оставлять"). Количество в формуле
        # итога не участвует: вход и возврат -- это SOL, и оба известны по
        # цепи. Не считается только одно: продажа по остатку кошелька, когда в
        # то же окно висела другая позиция ТОГО ЖЕ минта -- возврат тогда
        # принадлежит не только этой паре.
        if вход and возврат is not None and not п.get("uncountable_shared_mint"):
...
class ExecState:
    ...
    def _учесть_закрытие(self, cid: str, поля: dict) -> dict:
        ...
        слитое = dict(прежние)
        слитое.update(поля)
        # I.4: несчитаемой пару делает неизвестное КОЛИЧЕСТВО, а не деньги.
        # Итог по возврату и входу количества не требует вовсе. Единственный
        # случай, когда так считать нельзя, -- продажа по остатку кошелька,
        # когда в то же время на кошельке висела ДРУГАЯ позиция того же минта:
        # возврат тогда принадлежит не только этой паре.
        if слитое.get("result_uncountable"):
            общий = self._делит_минт(cid, слитое, все_позиции)
            if общий:
                слитое["uncountable_shared_mint"] = общий
    ...
    def _делит_минт(self, cid: str, поз: dict, все: dict) -> str:
        """Делегат к модульной функции: одна формула на весь репозиторий."""
        return делит_минт(cid, поз, все)

    def итог_позиции(self, поз: dict) -> tuple:
        """Делегат к модульной функции: одна формула на весь репозиторий.

        Отдельного написания у состояния быть не должно: ровно та же формула
        нужна суточному счёту полосы (bloom_own_send.состояние_полосы), и
        когда их было две, числа расходились.
        """
        return итог_позиции(поз)
    ...
    def can_open_detailed(self, *, mint: str, source_sig: str,
        ...
        if убит_б:
            # СВОЙ КОД: он говорит вызывающему, что запрет касается ТОЛЬКО
            # площадки. Полоса по этому сигналу идти обязана -- у неё свои
            # рубильник (KILL_OWN_SEND) и свои пределы.
            return False, почему_б, КОД_РУБИЛЬНИК_BLOOM
...
def self_test() -> None:
    ...
    # КОД У ЭТОГО ОТКАЗА СВОЙ (правка 26.09 по слову владельца): по общему коду
    # детектор глушил и полосу, потому что решение становилось skip, а полоса
    # запускается только на buy. Замер окна KILL_BLOOM 14:05:50-14:16:00Z:
    # отказов площадки 7, покупок полосы 0.
    chk("и гейт покупок площадки его слушает СВОИМ кодом",
        ок_б is False and код_б == КОД_РУБИЛЬНИК_BLOOM
        and код_б != КОД_РУБИЛЬНИК, (ок_б, код_б, почему_гейт))
    chk("а ОБЩИЙ рубильник при этом не включён -- полоса не остановлена",
        st.kill_active()[0] is False, st.kill_active())
    # И наоборот: при ОБЩЕМ рубильнике код прежний (KILL_SWITCH), потому что
    # этот запрет касается всех -- и площадки, и полосы.
    st.kill_path.parent.mkdir(parents=True, exist_ok=True)
    st.kill_path.write_text("стоп всем", encoding="utf-8")
    ок_о, _, код_о = st.can_open_detailed(balance_sol=10.0, mint="MO", source_sig="SO")
    chk("общий рубильник даёт ПРЕЖНИЙ код -- он глушит и полосу",
        ок_о is False and код_о == КОД_РУБИЛЬНИК, (ок_о, код_о))
    st.kill_path.unlink()
    ...
    # I.4. НЕСЧИТАЕМАЯ ПАРА С ИЗВЕСТНЫМИ ДЕНЬГАМИ СЧИТАЕТСЯ. Несчитаемой её
    # делает неизвестное КОЛИЧЕСТВО, а количество в формуле итога не
    # участвует: вход и возврат -- это SOL. Живая пара 25.09 18:09:41Z: вход
    # 0.05, вернулось 0.048637848, чаевые 0.001, приоритет 0.001, тариф
    # 0.000005 -> итог -0.003367152. Прежний счёт давал -0.002005, то есть
    # прятал убыток входа: за ночь таких пар 61 из 63 закрытых.
    сч2.write_intent(client_order_id="н1", mint="MN", source_sig="SN",
                     source_slot=4, sol_in=0.05, pool=None, program=None,
                     taxed=None, tax_bps=None, mode=MODE_LIVE, sell_after_s=28.8)
    стр_н = сч2.update_position("н1", state=STATE_CLOSED,
                                 ...
                                 closed_reason="продажа по остатку кошелька")
    chk("несчитаемая пара с известными входом и возвратом считается по ним",
        abs((стр_н.get("pnl_counted_sol") or 0) + 0.003367152) < 1e-9,
        стр_н.get("pnl_counted_sol"))

    # I.4, ОГОВОРКА: тот же минт держала ещё одна позиция полосы. Продажа шла
    # по остатку кошелька, значит возврат принадлежит обеим -- разложить
    # нечем, и в счёт идёт только расход.
    сч3 = ExecState(base=base / "pnl_shared", kill=base / "pnl_shared" / "НЕТ")
    сч3.write_intent(client_order_id="ш_сосед", mint="MS", source_sig="SS1",
                     source_slot=5, sol_in=0.01, pool=None, program=None,
                     taxed=None, tax_bps=None, mode=MODE_LIVE, sell_after_s=28.8)
    сч3.update_position("ш_сосед", lane="own_send", ts_intent=1000.0)
    сч3.write_intent(client_order_id="ш1", mint="MS", source_sig="SS2",
                     source_slot=6, sol_in=0.01, pool=None, program=None,
                     taxed=None, tax_bps=None, mode=MODE_LIVE, sell_after_s=28.8)
    стр_ш = сч3.update_position("ш1", state=STATE_CLOSED,
                                 closed_sol_net=0.011,
                                 result_uncountable=True,
                                 lane="own_send",
                                 ts_intent=1001.0, ts_closed=1002.0,
                                 lane_tips_total_sol=0.001,
                                 lane_priority_lamports=1_000_000,
                                 lane_signature="ПОДПИСЬ_Ш",
                                 closed_reason="продажа по остатку кошелька")
    chk("общий минт с другой позицией полосы -- в счёт только расход",
        abs((стр_ш.get("pnl_counted_sol") or 0) + 0.002005) < 1e-9,
        стр_ш.get("pnl_counted_sol"))

    # РАСХОД ПО ЦЕПИ СИЛЬНЕЕ РАСХОДА ПО ПОЛЯМ. Живая пара 25.09 18:09:41Z:
    # кошелёк отдал 0.053493440 (из них 0.001005 комиссия), вернулось
    # 0.048637848. Честный итог -0.004855592; по полям вышло бы -0.003367152,
    # потому что поля не видят платы за создание счетов (0.001488440).
    сч3.write_intent(client_order_id="ц1", mint="MC", source_sig="SC",
                     source_slot=8, sol_in=0.05, pool=None, program=None,
                     taxed=None, tax_bps=None, mode=MODE_LIVE, sell_after_s=28.8)
    стр_ц = сч3.update_position("ц1", state=STATE_CLOSED,
                                 closed_sol_net=0.048637848,
                                 lane="own_send",
                                 lane_buy_native_sol=-0.05248844,
                                 lane_buy_fee_sol=0.001005,
                                 lane_tips_total_sol=0.001,
                                 lane_priority_lamports=1_000_000,
                                 lane_signature="ПОДПИСЬ_Ц",
                                 closed_reason="продажа подтверждена по цепи")
    chk("итог считается по цепи: -0.004855592, а не -0.003367152 по полям",
        abs((стр_ц.get("pnl_counted_sol") or 0) + 0.004855592) < 1e-9,
        стр_ц.get("pnl_counted_sol"))
    chk("и расход по цепи 0.003493440 -- с платой за счета, а не 0.002005",
        abs((стр_ц.get("pnl_counted_spend_sol") or 0) - 0.00349344) < 1e-9,
        стр_ц.get("pnl_counted_spend_sol"))

    # I.4, ВТОРАЯ ОГОВОРКА: покупка ОТКЛОНЕНА пулом. Чаевые переводятся
    # инструкцией внутри той же транзакции, значит с упавшей они не уходят.
    # Живая сделка 26.09 00:04:23Z: вход 0.05, транзакция отклонена
    # (Custom 6040), списано ровно 0.001005 -- приоритет плюс тариф.
    сч3.write_intent(client_order_id="о1", mint="MO", source_sig="SO",
                     source_slot=7, sol_in=0.05, pool=None, program=None,
                     taxed=None, tax_bps=None, mode=MODE_LIVE, sell_after_s=28.8)
    стр_о = сч3.update_position("о1", state=STATE_CLOSED, closed_sol_net=0.0,
                                 result_uncountable=True, chain_ok=False,
                                 lane="own_send",
                                 lane_tips_total_sol=0.001,
                                 lane_priority_lamports=1_000_000,
                                 lane_signature="ПОДПИСЬ_О",
                                 closed_reason="продавать нечего: остаток 0")
    chk("у отклонённой покупки чаевые в расход НЕ идут: 0.001005, не 0.002005",
        abs((стр_о.get("pnl_counted_spend_sol") or 0) - 0.001005) < 1e-9,
        стр_о.get("pnl_counted_spend_sol"))
