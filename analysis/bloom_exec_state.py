...
class ExecState:
    ...
    def _делит_минт(self, cid: str, поз: dict, все: dict) -> str:
        ...
        Окна сравниваются по ts_intent (открытие) и ts_closed (закрытие); у ещё
        не закрытой позиции конец окна -- бесконечность, а не "сейчас": она
        всё ещё держит тот же минт.
        """
        минт = поз.get("mint")
        if not минт:
            return ""
        начало = float(поз.get("ts_intent") or 0.0)
        конец = float(поз.get("ts_closed") or 0.0) or float("inf")
        for чужой, о in (все or {}).items():
            # Чужая полоса не в счёт: у Bloom свой кошелёк, и его остаток
            # к остатку полосы отношения не имеет.
            if чужой == cid or о.get("lane") != поз.get("lane"):
                continue
            if о.get("mint") != минт:
                continue
    ...
    def итог_позиции(self, поз: dict) -> tuple:
        ...
        п = поз or {}
        расход = 0.0
        # ЧАЕВЫЕ УХОДЯТ ТОЛЬКО С УДАВШЕЙСЯ ТРАНЗАКЦИИ. Перевод чаевых -- это
        # инструкция внутри той же транзакции: упала транзакция -- перевода не
        # было, платится только приоритет и базовый тариф. Живой пример
        # 26.09 00:04:23Z: покупка 0.05 отклонена пулом (Custom 6040), с
        # кошелька полосы списано ровно 0.001005 SOL (приоритет 1 000 000
        # лампортов плюс тариф 5 000), а чаевые 0.001 остались на месте.
        # Считать их значит выдумывать убыток и приближать стоп на пустом
        # месте.
        ...
        # откатываются все переводы: ни 0.05 входа, ни чаевых с кошелька не
        # списано, платится только приоритет и тариф. Поэтому итог тут -- ровно
        # минус расход, и вычитать вход нельзя: это был бы выдуманный убыток
        # размером со сделку (26.09 00:04:23Z -- -0.051005 вместо -0.001005).
        ...
        if п.get("result_uncountable"):
            ...
            # возврату и входу; несчитаемых не оставлять"). Количество в
            # формуле итога не участвует: вход и возврат -- это SOL, и оба
            # известны по цепи. Поэтому несчитаемая пара с известными входом и
            # возвратом СЧИТАЕТСЯ, а не списывается в один расход.
            #
            # Не считается только одно: когда продажа шла по остатку кошелька и
            # в то же время висела другая позиция ТОГО ЖЕ минта -- возврат
            # принадлежит не только этой паре. Такой случай назван явно
            # (uncountable_shared_mint), и тогда в счёт идёт один расход.
