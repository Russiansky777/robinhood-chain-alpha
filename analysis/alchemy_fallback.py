...
_RATE_LIMIT_WAIT_BUDGET_S = 900.0  # 15 минут суммарного ожидания на 429 для ОДНОГО запроса, прежде чем сдаться на этом эндпоинте (ПО УМОЛЧАНИЮ, для исторических/аналитических задач -- см. set_rate_limit_wait_budget_s ниже про живой торговый путь)
_BACKOFF_CAP_S = 25.0
...
def set_rate_limit_wait_budget_s(value: float) -> float:
    """Пункт 8 (внешнее ревью, четвёртый раунд): "убрать исторические
    ретраи из торгового пути -- внешнее ожидание рецепта на N секунд не
    является реальной верхней границей, если ОДИН внутренний RPC-вызов
    способен ждать до _RATE_LIMIT_WAIT_BUDGET_S=900с НА КАЖДОМ из
    нескольких эндпоинтов (в сумме -- до ~45 минут на ОДИН вызов
    _rpc_call)". Это МОДУЛЬНЫЙ (не per-call) глобал -- каждый скрипт
    проекта запускается СВОИМ отдельным процессом (project-wide
    конвенция), поэтому мутация здесь НЕ меняет поведение остальных
    (исторических/аналитических) задач НИКАК -- она видна только внутри
    ЭТОГО процесса. task5_v4_hotpath.py вызывает эту функцию ОДИН раз в
    самом начале (main()), устанавливая ГОРАЗДО меньший бюджет
    (см. LIVE_RPC_RETRY_BUDGET_S), согласованный с реальными дедлайнами
    торгового пути (UNRESOLVED_RECOVERY_TIMEOUT_S, единый гейт остановки)
    -- НЕ повышает нагрузку на провайдера (сам троттлинг _MIN_REQUEST_
    INTERVAL_S/backoff-кап НЕ меняются, только СРОК СДАЧИ при устойчивой
    ошибке). Возвращает ПРЕЖНЕЕ значение (для восстановления в тестах)."""
    global _RATE_LIMIT_WAIT_BUDGET_S
    previous = _RATE_LIMIT_WAIT_BUDGET_S
    _RATE_LIMIT_WAIT_BUDGET_S = value
    return previous
