...
GROUP_OF = {
    # Службы исполнителя Bloom. Своего дневного бюджета у них ПОКА НЕТ:
    # назначать его самому значило бы выдать свою догадку за решение
    # владельца. Расход считается и докладывается, а порог владелец
    # назовёт сам -- до тех слов доля_бюджета у них остаётся пустой.
    "bloom_detector": None,
    "bloom_seller": None,
    "горизонты": "прогоны",
