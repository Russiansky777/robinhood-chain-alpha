...
def сигнал_пула_источника(helius, *, минт: str, tx_источника: dict | None,
    ...
    if разрыв > допуск_слотов:
        return dict(если_нет, pool=адрес, gap_slots=разрыв,
                     why_not=(f"ближайшая сделка через {разрыв} слотов после нашего "
                               f"выхода -- дальше допуска {допуск_слотов}"))
    ...
    return {"known": True, "level": "пул источника", "pool": адрес,
...
def пара(helius, поз: dict, *, кошелёк_dbot: str | None,
    ...
    # Если наш уровень сигнала не считается (вход и выход разными пулами) --
    # даём замену по пулу источника и помечаем, что это другой уровень.
    if not (итог["our"]["levels"].get("signal") or {}).get("known"):
        ...
        итог["our"]["signal_source_pool"] = сигнал_пула_источника(
            helius, минт=минт, tx_источника=tx_ист,
            кошелёк_источника=поз.get("source"),
            слот_выхода=(поз.get("closed_slot") or поз.get("our_slot")))
...
def self_test() -> None:
    ...
    зам_д = сигнал_пула_источника(h_ист, минт=МИНТ, tx_источника=tx_ист,
                                   ...
                                   допуск_слотов=2)
    chk("сделка дальше допуска заменой не выдаётся",
        зам_д["known"] is False and "допуска" in зам_д["why_not"], зам_д)
