...
# ПРАВКА (владелец, быстрый предварительный отбор: "их статус -- 'модель
# отсутствует', а не 'невыгодно'"): для маршрутов с хуком, чей (pool_id,
# zero_for_one) НЕ входит в CONFIRMED_HOOK_MODELS (task5_v4_pool_state_
# cache.py) -- используется ТОЛЬКО когда такой кандидат вытеснен из
# ограниченной очереди пересмотра НЕ дождавшись реального расчёта (реально
# оценённые кандидаты, включая эти, пишут СВОЙ настоящий REASON_*, этот --
# только для честной пометки "не считали вовсе", не "посчитали и не вышло").
REASON_HOOK_MODEL_ABSENT = "модель хука отсутствует (не оценивалось)"
# ПРАВКА (владелец: "неактивный маршрут не должен... занимать главную
# очередь через перепроверку живучести -- направь в ограниченный фоновый
# путь"): та же честная семантика, что REASON_HOOK_MODEL_ABSENT выше --
# используется ТОЛЬКО когда кандидат вытеснен из ограниченной очереди
# пересмотра живучести, не дождавшись самой перепроверки.
REASON_LIVENESS_RECHECK_DEFERRED = "перепроверка живучести отложена (не выполнялась)"
# ПРАВКА (владелец, разбор InsufficientFunds route_7aebd805z0...,
# 2026-09-14): подтверждено, что положительная ДО газа котировка может
# устареть за время в очереди -- повторная котировка на согласованном
# блоке (та, что идёт ПЕРЕД estimateGas) сама может оказаться
# неприбыльной. Раньше в этом случае код всё равно вызывал estimateGas
# (лишний RPC-вызов на заведомо неприбыльный на своём же блоке цикл --
# owner: "исправь один конкретный лишний вызов"). Эта причина -- ТОЛЬКО
# для этой конкретной точки (повторная котировка сама <= 0, до
# estimateGas), НЕ путать с REASON_NO_PROFITABLE_CYCLE (который
# используется и для "профит после газа <= 0", когда estimateGas
# РЕАЛЬНО был вызван и прошёл).
REASON_REQUOTE_UNPROFITABLE = "повторная котировка стала неприбыльной"
