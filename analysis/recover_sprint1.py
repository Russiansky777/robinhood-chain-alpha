...
НЕ ИСПОЛЬЗУЕТСЯ текущей ревизией analysis/sprint_1_5.py (ревизия 2, см.
docs/COST_POSTMORTEM.md): выяснилось, что чтение результата через API
биллится отдельно от execute() по объёму данных -- полный построчный
результат 03 (1.21M строк) стоил 163.98 кредита ЗА ОДНО чтение,
уровень 2 recover_baseline() ниже сделал бы ровно это. Архитектура
Sprint 1.5 переписана так, что гейт 1/2/капы/когорты считаются в SQL
на Dune (sql/03b_cohort_selection.sql и соседние), и наружу через API
едут только уже отфильтрованные, маленькие результаты -- см.
sprint_1_5.py. Модуль оставлен как справочная реализация паттерна
"переиспользовать уже оплаченный execution_id вместо пересчёта" на
случай, если он снова понадобится для ДЕЙСТВИТЕЛЬНО маленького
результата.
