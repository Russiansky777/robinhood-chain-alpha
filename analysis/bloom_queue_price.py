...
# ------------------------------ каким путём пришёл сигнал (запасной или нет)

# Поле "via" в строке решения -- это СПОСОБ ПОДПИСКИ, которым сигнал пришёл:
# transactionSubscribe (основной) или logsSubscribe (запасной). Оно пишется
# детектором с самого начала, поэтому вопрос владельца "был ли детектор на
# запасном в момент сигнала" отвечается по УЖЕ НАКОПЛЕННОМУ журналу, без
# новых полей и без единого вызова сети.
ОСНОВНОЙ_ПУТЬ = "transactionSubscribe"
ЗАПАСНОЙ_ПУТЬ = "logsSubscribe"


def путь_сигналов(строки: list) -> dict:
    """Подпись источника -> каким путём этот сигнал пришёл.

    Берётся ПЕРВАЯ запись по подписи: сигнал приходит один раз, а дальше по
    той же подписи идут строки разбора, и в них via может отсутствовать.
    """
    из_: dict = {}
    for с_ in строки:
        с_ = с_.strip()
        if not с_ or '"via"' not in с_:
            continue
        try:
            р = json.loads(с_)
        except ValueError:
            continue
        подпись, путь = р.get("signature"), р.get("via")
        if not подпись or not путь or подпись in из_:
            continue
        из_[подпись] = путь
    return из_


def влияние_запасного(итоги: list) -> dict:
    """Доля S+0 у покупок с основного пути и с запасного.

    Вопрос владельца: если покупки на запасном чаще дают S+1/S+2, это
    отдельная причина отставания. Считается только по тем покупкам, где путь
    известен: приписывать неизвестному пути основной значило бы улучшить
    картину выдумкой.
    """
    свои = [и for и in итоги if и.get("known") and и.get("via")]
    def доля_s0(ряд):
        сели = [и for и in ряд if isinstance(и.get("slots_behind"), int)]
        if not сели:
            return None
        return round(len([и for и in сели if и["slots_behind"] == 0]) / len(сели), 4)
    основной = [и for и in свои if и["via"] == ОСНОВНОЙ_ПУТЬ]
    запасной = [и for и in свои if и["via"] == ЗАПАСНОЙ_ПУТЬ]
    из_ = {"n_known_path": len(свои), "n_main": len(основной),
           "n_fallback": len(запасной),
           "share_s0_main": доля_s0(основной),
           "share_s0_fallback": доля_s0(запасной),
           "n_unknown_path": len([и for и in итоги
                                   if и.get("known") and not и.get("via")])}
    def медиана_слотов(ряд):
        сл = sorted(и["slots_behind"] for и in ряд
                    if isinstance(и.get("slots_behind"), int))
        return (сл[len(сл) // 2] if сл else None)
    из_["slots_behind_median_main"] = медиана_слотов(основной)
    из_["slots_behind_median_fallback"] = медиана_слотов(запасной)
    return из_
...
def таблица_доли(с: dict, *, наш_расход_мс: float | None = None) -> str:
    ...
    строки = ["", "## Момент источника внутри слота (пункт 2а)", "",
              ...
              "наша плата, SOL | путь подписки | чем доставлена наша |",
              "|---|---|---|---|---|---|---|"]
    for р in с["rows"]:
        путь = р.get("via")
        путь_текст = ("основной" if путь == ОСНОВНОЙ_ПУТЬ else
                       ("ЗАПАСНОЙ" if путь == ЗАПАСНОЙ_ПУТЬ else "не знаем"))
        строки.append(
            f"| {р['source_share']} | {р['source_index']}/{р['source_block_total']} | "
            f"S+{р['slots_behind']} | {р['our_index']}/{р['our_total_in_block']} | "
            f"{р['our_pay_sol']} | {путь_текст} | {', '.join(р['accelerator'])} |")
