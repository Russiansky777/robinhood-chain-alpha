...
def min_out_from(program: str, data_b58: str) -> dict:
    ...
    elif program == JUPITER_V6 and len(d) >= 28:
        # Раскладка подтверждена тремя независимыми сходимостями на живых
        # транзакциях: вход совпал с фактически отданной суммой до знака,
        # комиссия площадки в bps совпала с фактически удержанной суммой,
        # ожидаемый выход близок к фактически полученному.
        # 8 байт дискриминатора, 1 байт номера, затем вход, ожидаемый выход,
        # проскальзывание в bps и комиссия площадки в bps.
        slippage = int.from_bytes(d[25:27], "little")
        out.update(вид="Jupiter v6", вход=u64(d, 9), ожидаемый_выход=u64(d, 17),
                   проскальзывание_bps=slippage, комиссия_площадки_bps=d[27])
        q = u64(d, 17)
        if q is not None:
...
def check_min_out(разбор: dict, факт_вход_raw: int | None) -> dict:
    """Раскладку нельзя принимать на веру: сверяем вход из инструкции с тем,
    что трейдер реально отдал. Не сошлось -- цифры помечаются негодными."""
    if not разбор or разбор.get("вход") is None or факт_вход_raw is None:
        разбор = dict(разбор or {})
        разбор["сверка_раскладки"] = "нечего сверять"
        return разбор
    ok = abs(разбор["вход"] - факт_вход_raw) <= max(1, факт_вход_raw // 1000)
    разбор = dict(разбор)
    разбор["сверка_раскладки"] = ("вход из инструкции совпал с фактически отданным"
                                   if ok else
                                   f"вход из инструкции {разбор['вход']} не совпал с фактически "
                                   f"отданным {факт_вход_raw}: раскладка не та, цифрам верить нельзя")
    разбор["раскладка_подтверждена"] = ok
    if not ok:
        for k in ("вход", "ожидаемый_выход", "проскальзывание_bps", "минимальный_выход",
                   "комиссия_площадки_bps"):
            if k in разбор:
                разбор[k + "_негодно"] = разбор.pop(k)
    return разбор


def withheld(pool_delta: float, our_delta: float) -> dict:
    """Сколько токена пропало МЕЖДУ пулом и кошельком. У обычного токена
    это ноль; у Token-2022 с комиссией перевода -- она самая."""
    if not pool_delta or not our_delta:
        return {"почему": "нет одной из ног"}
    доля = abs(our_delta) / abs(pool_delta)
    return {"дошло_доля": round(доля, 8), "удержано_pct": round((1 - доля) * 100, 4)}
...
def main() -> None:
    ...
    for label, sig in (("покупка", BUY_SIG), ("продажа", SELL_SIG)):
        ...
        s["наша_нога"] = tr
        if p.get("ок"):
            # Комиссия на перевод: сколько токена не доехало между пулом и
            # кошельком. У обычного токена ноль, у Token-2022 -- ставка.
            s["удержано_на_переводе_целевого_токена"] = withheld(
                p.get("дельта_токена"), (tr or {}).get("токенов", 0) * (1 if tr and
                tr["направление"] == "покупка" else -1))
            ins = None
            for i in ((tx.get("transaction") or {}).get("message") or {}).get("instructions") or []:
                if i.get("programId") == JUPITER_V6 and i.get("data"):
                    ins = i
                    break
            if ins is None:
            ...
            if ins:
                ...
                # Чем трейдер платил -- тем и сверяем раскладку.
                факт = None
                if tr:
                    факт = (round((tr.get("sol") or 0) * 1e9) if tr["направление"] == "покупка"
                            else None)
                s["инструкция_обмена"] = {"программа": ins.get("programId"),
                                           "разбор": check_min_out(разбор, факт)}
            else:
                s["инструкция_обмена"] = {"почему": "инструкция обмена с данными не найдена"}
        стороны[label] = s
        rep["стороны"][label] = s
    ...
    if buy.get("слот") and sell.get("слот"):
        ...
        if наша_продажа is not None and sell.get("blockTime"):
            ...
            if до:
                ...
                курс = (sell.get("пул") or {}).get("курс_котировки_в_sol")
                # Цена пула -- в валюте пула, наша цена -- в SOL. Сравнивать
                # их напрямую нельзя: получится разница в десятки раз ни о чём.
                if наша and кпосле and курс:
                    в_sol = кпосле * курс
                    rep["последняя_цена_пула_в_sol"] = в_sol
                    rep["наша_цена_выхода_против_последней_цены_пула_pct"] = round(
                        (наша / в_sol - 1) * 100, 3)
                    rep["оговорка_к_сравнению"] = (
                        "цена пула переведена в SOL по курсу промежуточного токена из той же "
                        "транзакции; разрыв -- это комиссии на перевод и комиссия площадки "
                        "по дороге от пула до кошелька")
                elif наша and кпосле:
                    rep["наша_цена_выхода_против_последней_цены_пула_pct"] = None
                    rep["почему_нет_сравнения"] = (
                        "цена пула в валюте пула, наша -- в SOL, а курса из транзакции нет: "
                        "сравнивать разные валюты нельзя")
            else:
                rep["последняя_сделка_до_продажи"] = {
                    "почему": "за 5 секунд до нашей продажи в этом пуле сделок не было"}

    # Разложение итога на три части. Проверка честности -- произведение
    # частей обязано сойтись с фактическим итогом.
    pb, ps = buy.get("пул") or {}, sell.get("пул") or {}
    if pb.get("ок") and ps.get("ок") and pb.get("средняя_цена_пула") and ps.get("средняя_цена_пула"):
        ход = ps["средняя_цена_пула"] / pb["средняя_цена_пула"]
        курс = ((ps.get("курс_котировки_в_sol") or 0) / (pb.get("курс_котировки_в_sol") or 1)
                if pb.get("курс_котировки_в_sol") and ps.get("курс_котировки_в_sol") else None)
        уд_b = (buy.get("удержано_на_переводе_целевого_токена") or {}).get("дошло_доля")
        уд_s = (sell.get("удержано_на_переводе_целевого_токена") or {}).get("дошло_доля")
        rep["разложение"] = {
            "движение_цены_в_пуле_pct": round((ход - 1) * 100, 3),
            "курс_промежуточного_токена_pct": round((курс - 1) * 100, 3) if курс else None,
            "дошло_целевого_токена_покупка": уд_b,
            "дошло_целевого_токена_продажа": уд_s,
            "оговорка": ("движение цены и курс -- по ногам самих пулов; комиссии на перевод "
                          "считаются отдельно по каждому переводу каждого токена маршрута"),
        }
