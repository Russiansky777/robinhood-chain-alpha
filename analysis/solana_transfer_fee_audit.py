...
def route_of(tx: dict, target: str) -> dict:
    ...
    return {"токены": токены, "промежуточные": sorted(set(промежуточные)),
             "удержано_всего_sol": round(удержано_sol, 9),
...
def self_test() -> None:
    ...
    chk("и в SOL ноль, а не пусто", r2["удержано_всего_sol"] == 0.0)
...
def main() -> None:
    ...
    по_задачам = defaultdict(lambda: {"сделок": 0, "через_таксируемый": 0, "удержано_sol": 0.0,
                                       "сумма_по_цепи_sol": 0.0, "без_курса": 0})
    всего = {"сделок": 0, "через_промежуточный": 0, "удержано_sol": 0.0, "без_курса": 0}
    строки = []
    for t in trades:
        ...
        if not rb or not rs:
            continue
        mint = t["mint"]
        межд = sorted({m for r in (rb, rs) for m in r["промежуточные"] if m != mint})
        уд = 0.0
        нет_курса = False
        for r in (rb, rs):
        ...
        if межд:
            g["через_таксируемый"] += 1
            g["сумма_по_цепи_sol"] += цепь
            всего["через_промежуточный"] += 1
            for m in межд:
                промеж[m]["сделок"] += 1
                промеж[m]["сумма_по_цепи_sol"] += цепь
                промеж[m]["удержано_sol"] += уд
        строки.append({"задача": задача, "минт": mint, "промежуточные": межд,
                        "удержано_sol": round(уд, 9), "sol_in": t.get("sol_in"),
                        "gross_pct": t.get("gross_pct")})

    # 1. Минты: цели и все найденные промежуточные.
    минты = {}
    for m in [SANTA, PURPLE] + sorted(промеж):
        if m not in минты:
            минты[m] = mint_info(rpc, m)
    for имя, d in пары.items():
        for сд in d["сделки"]:
            for m in (сд.get("маршрут") or {}).get("промежуточные", []):
                if m not in минты:
                    минты[m] = mint_info(rpc, m)
    rep["минты"] = минты
    ...
    rep["свод"] = {
        ...
        "из_них_через_промежуточный_токен": всего["через_промежуточный"],
        "доля_через_промежуточный": (round(всего["через_промежуточный"] / всего["сделок"], 4)
        ...
        "оговорка_к_сумме": ("сделки без курса в сумму не попали -- их удержанное "
                              "не подставлено нулём, а исключено, и их число указано"),
    }
    ...
    rep["по_задачам"] = {
        k: {**v, "удержано_sol": round(v["удержано_sol"], 6),
            ...
            "доля_через_таксируемый": (round(v["через_таксируемый"] / v["сделок"], 4)
    ...
    краткое = {k: v for k, v in rep.items() if k not in ("строки", "пары")}
