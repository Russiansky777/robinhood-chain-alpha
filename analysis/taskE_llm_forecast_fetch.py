#!/usr/bin/env python3
"""ЗАБРОШЕНО (2026-09-10) -- НЕ ИСПОЛЬЗОВАТЬ. Реальный инцидент: файловое
разделение questions_sealed/outcomes_sealed полагалось на ДИСЦИПЛИНУ
(не открывать второй файл раньше времени), а не на структурную защиту --
харнесс-механизм отслеживания изменений файлов автоматически показал
полное содержимое outcomes_sealed.json, когда файл изменился на диске,
потому что этот файл был однажды прочитан (даже пустым, `cat` на первом
бракованном прогоне) на более раннем шаге. Слепой тест на той выборке
скомпрометирован. Владелец распорядился пересобрать с временным (не
файловым) разделением -- см. taskE_step1_context_fetch.py (Шаг 1,
код физически не ссылается на outcome-поля вообще) и
taskE_step2_score.py (Шаг 2, отдельный ПОЗДНИЙ запуск, только после
того как коммит Шага 1 виден в git; выдаёт только готовый Brier score,
не сырые исходы). Этот файл оставлен как есть для истории/паспорта,
