...
def в_текст(о: dict) -> str:
    ...
    строки = [
        ...
        f"{json.dumps(реш.get('by_code') or {}, ensure_ascii=False)}. Полоса идёт "
        "только там, где мы покупаем, поэтому её тишина объясняется этой строкой.",
        "",
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        (сост / "bloom_detector_heartbeat.json").write_text(
            json.dumps({"updated_utc": "2026-09-25T02:00:00Z",
                        "own_send": {"live": True}}, ensure_ascii=False),
            encoding="utf-8")
