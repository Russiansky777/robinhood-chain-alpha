...
# ГОЛОВА БЛОКА. Уточнение владельца 25.09: цель -- S+0 в любом месте или
# голова S+1, а голова это место не дальше сотого. Число здесь, а не по тексту:
# в докладах доля считается именно по нему.
ГОЛОВА_БЛОКА = 100
...
def слоты_и_места(позиции: dict) -> dict:
    ...
    for имя, отбор in (("bloom", lambda п: not п.get("lane")),
        ...
        # ЦЕЛЬ ПО МЕСТУ (уточнение владельца 25.09): "не первое место после
        # источника, а S+0 в любом месте или голова S+1 (место <= 100)". Считаем
        # именно так и отдельным числом: доля S+0 сама по себе этого не
        # показывает, а доля "первых мест" показывает не то, что нужно.
        в_цели = 0
        цель_есть = 0
        for п in свои:
            их = п.get("source_slot")
            наш = п.get("our_slot") or п.get("own_tx_seen_slot")
            место = п.get("block_index")
            if not (isinstance(их, int) and isinstance(наш, int)):
                continue
            отставание = наш - их
            if отставание == 0:
                # S+0 в ЛЮБОМ месте блока -- цель достигнута, место не важно.
                цель_есть += 1
                в_цели += 1
            elif отставание == 1:
                # S+1 -- только голова блока. Без места судить нельзя, и такие
                # пары в знаменатель не идут: иначе отсутствие места читалось бы
                # как промах.
                if isinstance(место, int):
                    цель_есть += 1
                    if место <= ГОЛОВА_БЛОКА:
                        в_цели += 1
            else:
                цель_есть += 1
        из_[имя] = {"positions": len(свои), "with_slots": сколько,
                    ...
                    # ЦЕЛЬ: S+0 в любом месте ИЛИ S+1 с местом <= 100.
                    "goal_hits": в_цели, "goal_judged": цель_есть,
                    "goal_head_place": ГОЛОВА_БЛОКА,
                    "share_goal": (round(в_цели / цель_есть, 4)
                                    if цель_есть else None)}
    return из_
...
def в_текст(о: dict) -> str:
    ...
    строки = [
        ...
        ("| исполнитель | сделок со слотами | В ЦЕЛИ (S+0 или голова S+1) | "
          "S+0 | S+1 | S+2 | S+3 и дальше | медиана отставания | "
          "медиана места | медиана доли места |"),
        "|---|---|---|---|---|---|---|---|---|---|",
        *[f"| {имя} | {ч(з.get('with_slots'))} | "
          f"{ч(з.get('share_goal'))} ({ч(з.get('goal_hits'))} из "
          f"{ч(з.get('goal_judged'))}) | {ч(з.get('share_s0'))} | "
        ...
        "",
        ("Цель по месту (слово владельца 25.09): S+0 в ЛЮБОМ месте блока или "
          "голова S+1 -- место не дальше сотого. Пары S+1 без известного места в "
          "знаменатель не идут: отсутствие места -- это не промах."),
        "",
        "**Итог по цепи.**",
