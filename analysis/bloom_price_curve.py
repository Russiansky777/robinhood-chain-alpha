...
_ПРОГРАММЫ_ПУЛОВ: dict | None = None


def программы_пулов() -> dict:
    """Программы пулов для РАЗБОРА КУРСА: свой короткий список плюс реестр.

    У детектора список из десяти программ, и он короткий намеренно: по нему
    принимаются решения в горячем пути, и чем он уже, тем меньше шансов
    посчитать за пул то, что пулом не является. Для доклада этого мало: на
    десяти парах 24.09 замена уровня "сигнал" отказала восемь раз, и разбор
    отказа показал ровно это -- сделка источника шла через Raydium Launchlab
    и Meteora DAMM v2, которых в коротком списке нет.

    Поэтому здесь берётся data/.../dex_labels.json -- тот же реестр на 107
    программ пулов, который уже ездит на хост для тени. Он ТОЛЬКО читается и
    ТОЛЬКО здесь: горячий путь детектора и его решения не меняются ни на
    байт. Реестра нет на диске -- работаем по короткому списку и молчать об
    этом не будем: причина видна в отказе полем programs_unknown_to_us.
    """
    global _ПРОГРАММЫ_ПУЛОВ  # noqa: PLW0603
    if _ПРОГРАММЫ_ПУЛОВ is not None:
        return _ПРОГРАММЫ_ПУЛОВ
    из_ = dict(BD.ПРОГРАММЫ_DEX)
    путь = (Path(__file__).resolve().parents[1] / "data" / "solana_buyer_200"
            / "prior" / "current" / "buyer_100" / "dex_labels.json")
    try:
        реестр = json.loads(путь.read_text(encoding="utf-8"))
        if isinstance(реестр, dict):
            for адрес, имя in реестр.items():
                из_.setdefault(адрес, имя if isinstance(имя, str) else str(имя))
    except (OSError, ValueError):
        pass
    _ПРОГРАММЫ_ПУЛОВ = из_
    return из_
...
def курс_по_пулу(tx: dict, минт: str, *, кошелёк: str | None = None,
    ...
    for ins in _инструкции(tx):
        ...
        if хранилище in счета and программы_пулов().get(ins.get("programId")):
            набор.update(счета)
    if not набор:
        # Отказ без разбора причины -- тупик: непонятно, то ли хранилище не
        # то, то ли программа пула нам неизвестна. Поэтому в отказ кладётся,
        # ЧТО именно было в транзакции: встречалось ли хранилище вообще и
        # какие программы там стояли. По этим полям видно, чинить ли список
        # программ DEX или выбор хранилища.
        программы = []
        хранилище_встретилось = False
        for ins in _инструкции(tx):
            счета = [a for a in (ins.get("accounts") or []) if isinstance(a, str)]
            pid = ins.get("programId")
            if pid and pid not in программы:
                программы.append(pid)
            if хранилище in счета:
                хранилище_встретилось = True
        неизвестные = [x for x in программы if not программы_пулов().get(x)]
        return {"known": False, "vault": хранилище,
                 "why_not": "хранилище не встречается в счетах инструкций DEX",
                 "vault_seen_at_all": хранилище_встретилось,
                 "programs_seen": программы[:12],
                 "programs_unknown_to_us": неизвестные[:12]}
    ключи = _ключи(tx)
...
def self_test() -> None:
    ...
    # ОТКАЗ ДОЛЖЕН ОБЪЯСНЯТЬ СЕБЯ. 24.09 на десяти парах замена уровня
    # "сигнал" отказала восемь раз одной и той же строкой "хранилище не
    # встречается в счетах инструкций DEX", и по ней невозможно понять,
    # чинить ли выбор хранилища или список программ DEX. Теперь в отказе
    # лежат сами программы и признак, встречалось ли хранилище вообще.
    чужая = сделка(токенов=1000.0, sol=1.0)
    for ins in ((чужая.get("transaction") or {}).get("message") or {}).get("instructions") or []:
        ins["programId"] = "НеИзвестнаяПрограмма1111111111111111111111"
    отказ = курс_по_пулу(чужая, МИНТ, хранилище="ПУЛ-ХРАН-НЕТ")
    chk("отказ называет программы транзакции, а не только себя",
        отказ["known"] is False and отказ.get("programs_seen")
        and отказ.get("vault_seen_at_all") is False, отказ)
    отказ2 = курс_по_пулу(чужая, МИНТ, хранилище=(
        ((чужая.get("transaction") or {}).get("message") or {})
        .get("instructions")[0].get("accounts")[0]))
    chk("если хранилище в транзакции ЕСТЬ, но программа чужая -- это видно",
        отказ2["known"] is False and отказ2.get("vault_seen_at_all") is True
        and отказ2.get("programs_unknown_to_us"), отказ2)

    # РЕЕСТР ПРОГРАММ ПУЛОВ. Короткий список детектора и широкий список для
    # разбора курса -- разные вещи, и смешивать их нельзя: по первому
    # принимаются решения в горячем пути.
    ш = программы_пулов()
    chk("для разбора курса реестр шире короткого списка детектора",
        len(ш) > len(BD.ПРОГРАММЫ_DEX), (len(ш), len(BD.ПРОГРАММЫ_DEX)))
    chk("и в нём есть ровно те программы, на которых отказ и спотыкался",
        ш.get("LanMV9sAd7wArD4vJFi2qDdfnVhFxYSUg6eADduJ3uj")
        and ш.get("cpamdpZCGKUy5JxQXB4dcpGPiikHawvSWAd6mEn1sGG"),
        [ш.get("LanMV9sAd7wArD4vJFi2qDdfnVhFxYSUg6eADduJ3uj"),
         ш.get("cpamdpZCGKUy5JxQXB4dcpGPiikHawvSWAd6mEn1sGG")])
    chk("короткий список детектора при этом НЕ тронут",
        len(BD.ПРОГРАММЫ_DEX) == 10, len(BD.ПРОГРАММЫ_DEX))
