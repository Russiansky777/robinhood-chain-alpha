...
def пул_для_кривой(helius, *, минт: str, tx_источника: dict | None,
    ...
    пул = попытка("транзакция источника", tx_источника, кошелёк_источника)
    if пул:
        return {"pool": пул, "pool_from": "транзакция источника",
                 "candidates": диаг[:8], "why_not": None}
    if подпись_нашей:
        пул = попытка("наша покупка", helius.транзакция(подпись_нашей), кошелёк_наш)
        if пул:
            return {"pool": пул, "pool_from": "наша покупка",
                     "candidates": диаг[:8], "why_not": None}
    return {"pool": None, "pool_from": None, "candidates": диаг[:8],
...
def self_test() -> None:
    ...
    # --- выбор пула для кривой: источник, потом наша покупка, потом отказ ---
    RAY = "675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8"

    def тx_с_пулом(*, пул="ПУЛ2", кв=WSOL, в_dex=True, владелец_кошелёк=False,
                    плательщик="КОШ"):
        вл = "КОШ" if владелец_кошелёк else пул
        ins = [{"programId": RAY, "accounts": ([вл] if в_dex else ["ДРУГОЙ"])}]
        return {"slot": 100, "blockTime": 1790000000,
                 "transaction": {"signatures": ["X"], "message": {
                     "accountKeys": [{"pubkey": плательщик, "signer": True}],
                     "instructions": ins}},
                 "meta": {"err": None, "fee": 5000,
                           "preBalances": [10 ** 9], "postBalances": [10 ** 9],
                           "preTokenBalances": [бал(вл, МИНТ, 1000, 1),
                                                 бал(вл, кв, 1000, 2, 9)],
                           "postTokenBalances": [бал(вл, МИНТ, 900, 1),
                                                  бал(вл, кв, 1100, 2, 9)]}}
    ...
    chk("пул взят из транзакции источника, откуда -- сказано",
        в1["pool"] == "ПУЛ2" and в1["pool_from"] == "транзакция источника", в1)

    # В транзакции источника кандидат единственный и он же кошелёк сделки --
    # пула нет; тогда адрес берётся из НАШЕЙ покупки того же минта.
    h = HeliusНашаПокупка(тx_с_пулом(пул="ПУЛ3"))
    в2 = пул_для_кривой(h, минт=МИНТ,
                         tx_источника=тx_с_пулом(владелец_кошелёк=True),
                         кошелёк_источника="КОШ", подпись_нашей="НАША")
    chk("пула в транзакции источника нет -- берётся из нашей покупки",
    ...
    в4 = пул_для_кривой(HeliusНашаПокупка(тx_с_пулом(в_dex=False)), минт=МИНТ,
                         tx_источника=тx_с_пулом(в_dex=False),
                         кошелёк_источника="КОШ", подпись_нашей="НАША")
    chk("пула нет нигде -- причина по каждой транзакции, а не пустота",
    ...
    chk("в сводке кандидатов виден отклонённый и за что",
        в4["candidates"] and в4["candidates"][0]["rejected"]
        and в4["candidates"][0]["has_mint"] is True, в4["candidates"])
